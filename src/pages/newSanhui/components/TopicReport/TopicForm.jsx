...
export default function TopicForm(props) {
  ...
  const columns = [
    { title: "序号", dataIndex: "id", width: 60, render: (_value, _record, index) => index + 1 },
    {
      title: "职务分类",
    ...
    { title: "参会对象", dataIndex: "attendFlag", render: (value) => (value === "1" ? "√" : "-") },
    {
      title: "是否需要专题汇报",
    ...
    <>
      <div className="topic-report-grid">
        <div className="topic-report-card">
          ...
          <Table size="small" columns={columns} dataSource={tableData} rowKey="id" pagination={false} scroll={{ x: 760 }} />
        </div>
        <div className="topic-report-panel">
          <div className="topic-report-title">向集团总经理助理及以上董监事汇报</div>
          <DocumentFiling
            ref={documentFormRef}
