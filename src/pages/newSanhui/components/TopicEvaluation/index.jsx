import { DownloadOutlined, QuestionCircleOutlined, SendOutlined, UploadOutlined } from "@ant-design/icons";
import { Button, Drawer, Form, Input, Modal, Popconfirm, Radio, Select, Space, Switch, Table, Tabs, Tag, Tooltip, Upload, message } from "antd";
...
const pdfFileUrlMap = {
  ...
  "20260403(股权运营部)T3出行第二届董事会2026年第一次会议、2026年第三次临时股东会议案及表决建议.pdf": companyReviewSecondPdfUrl,
  "1.招标文件-备注版-明阳智能AI在多业务域应用试点项目-20250606.pdf": companyReviewDocumentPdfUrl,
};
...
const seedCompanyFeedbackRows = [
  {
    ...
    page: 1,
    feedbackContent: "关键净值与拆除费用表格区域，需要在评估前补充附件来源说明。",
    companyAnswer: "已补充固定资产卡片、处置评估底稿和拆除费用说明。",
    feedbackTime: "2025-06-21 19:05",
    answerTime: "2025-06-22 09:40",
    rounds: [
      { type: "send", role: "评估人员", content: "请参股公司补充净值表格与拆除费用的原始附件来源。", time: "2025-06-21 19:12" },
      { type: "reply", role: "参股公司", content: "已补充固定资产卡片、处置评估底稿和拆除费用说明。", time: "2025-06-22 09:40" },
      { type: "send", role: "评估人员", content: "请继续补充附件日期与评估报告引用页码，便于闭环归档。", time: "2025-06-22 10:15" },
    ],
  },
  {
    ...
    pdfName: "20250428中联电子议题关键信息页(1).pdf",
    page: 1,
    feedbackContent: "确认“无法再使用”的判断依据是否需要补充现场照片或附表说明。",
    companyAnswer: "已上传现场照片6张，并补充设备拆除验收单。",
    feedbackTime: "2025-06-21 19:08",
    answerTime: "2025-06-22 11:05",
    rounds: [
      { type: "send", role: "评估人员", content: "请补充设备现场照片和无法继续使用的判断依据。", time: "2025-06-21 19:18" },
      { type: "reply", role: "参股公司", content: "已上传现场照片6张，并补充设备拆除验收单。", time: "2025-06-22 11:05" },
    ],
  },
  {
    id: "company-feedback-discussion-1",
    annotationId: "discussion-1",
    topicName: "T3出行第二届董事会2026年第一次会议、2026年第三次临时股东会议案及表决建议",
    pdfName: "20260403(股权运营部)T3出行第二届董事会2026年第一次会议、2026年第三次临时股东会议案及表决建议.pdf",
    page: 2,
    feedbackContent: "建议在董事会审议前完成净值口径、处置价格依据及资产完备性说明的补充标注，并同步形成任务清单。",
    companyAnswer: "净值口径已按财务账面净值更新，处置价格依据已补充评估公司询价记录。",
    feedbackTime: "2026-05-15 18:06",
    answerTime: "2026-05-16 15:30",
    rounds: [
      { type: "send", role: "评估人员", content: "请说明净值口径、处置价格依据和资产完备性来源。", time: "2026-05-15 18:20" },
      { type: "reply", role: "参股公司", content: "净值口径已按财务账面净值更新，处置价格依据已补充评估公司询价记录。", time: "2026-05-16 15:30" },
      { type: "send", role: "评估人员", content: "请把三类附件拆分命名，并在议案正文中标出引用位置。", time: "2026-05-16 16:10" },
      { type: "reply", role: "参股公司", content: "已按附件1-3拆分上传，正文第2页和第5页已补充引用说明。", time: "2026-05-17 09:15" },
    ],
  },
];
...
function mergeSeedCompanyFeedbackRows(stored) {
  ...
  const mergedSeeds = seedCompanyFeedbackRows.map((seed) => ({
    ...
    rounds: storedMap.get(seed.id)?.rounds?.length ? storedMap.get(seed.id).rounds : seed.rounds,
  }));
  const extraItems = stored.filter((item) => !seedCompanyFeedbackRows.some((seed) => seed.id === item.id));
...
function writeCompanyFeedbackRows(rows) {
  ...
  window.dispatchEvent(new CustomEvent("newSanhui:companyFeedbackChange", { detail: rows }));
}

function writePdfAnnotations(annotations) {
  if (typeof window === "undefined") return;
  window.localStorage.setItem(annotationsStorageKey, JSON.stringify(annotations));
  window.dispatchEvent(new CustomEvent("newSanhui:pdfAnnotationsChange", { detail: annotations }));
}
...
function CompanyFeedbackTab({ topics }) {
  const [rows, setRows] = useState(() => mergeSeedCompanyFeedbackRows(readStorageList(companyFeedbackStorageKey, seedCompanyFeedbackRows)));
  ...
  const selectedTopicGroups = useMemo(() => {
    ...
    selectedRows.forEach((row) => {
      ...
      const topicGroup = topicGroups.get(topicName) || { topicName, pdfGroups: new Map(), rows: [] };
      const pdfGroup = topicGroup.pdfGroups.get(pdfName) || { pdfName, pdfUrl: getPdfFileUrl(pdfName), rows: [] };
  ...
  useEffect(() => {
    const handleChange = (event) => {
      setRows(mergeSeedCompanyFeedbackRows(Array.isArray(event.detail) ? event.detail : readStorageList(companyFeedbackStorageKey, seedCompanyFeedbackRows)));
    };
    window.addEventListener("newSanhui:companyFeedbackChange", handleChange);
    return () => window.removeEventListener("newSanhui:companyFeedbackChange", handleChange);
  }, []);

  const columns = [
    { title: "序号", width: 70, align: "center", render: (_, __, index) => index + 1 },
    { title: "议题名称", dataIndex: "topicName", minWidth: 240, render: (text) => <span className={styles.topicName}>{text}</span> },
    { title: "PDF名称", dataIndex: "pdfName", width: 240 },
    { title: "页签", dataIndex: "page", width: 86, align: "center", render: (page) => <Tag color="blue">第{page}页</Tag> },
    { title: "反馈内容", dataIndex: "feedbackContent", minWidth: 260 },
    {
      title: "参股公司回答",
      ...
      render: (value, record) => {
        ...
        return (
          <div className={styles.feedbackAnswerCell}>
            <span>{pending ? <span className={styles.mutedText}>待回复</span> : value || <span className={styles.mutedText}>待回复</span>}</span>
            <div className={styles.feedbackStatusTags}>
              {(record.rounds || []).length ? <Tag color="processing">{record.rounds.length}轮交互</Tag> : null}
              {pending ? <Tag color="warning">待回复</Tag> : <Tag color="success">已回复</Tag>}
            </div>
          </div>
        );
      },
    ...
    { title: "回答时间", dataIndex: "answerTime", width: 150, render: (value, record) => isPendingCompanyReply(record) ? "-" : value || "-" },
  ];
  ...
  const exportRows = () => {
    const header = ["id", "annotationId", "议题名称", "PDF名称", "页签", "反馈内容", "参股公司回答", "反馈时间", "回答时间"];
  ...
  const syncImportedRowsToAnnotations = (nextRows) => {
    ...
    const answerMap = new Map(nextRows.filter((item) => item.annotationId && item.companyAnswer).map((item) => [item.annotationId, item]));
    const nextAnnotations = annotations.map((item) => {
      ...
      return {
        ...
        discussion: discussion.includes(replyText) ? discussion : [...discussion, replyText],
      };
    });
  ...
  const beforeImport = (file) => {
    ...
    reader.onload = () => {
      try {
        ...
        message.success(`已导入 ${nextRows.length} 条参股公司反馈，并同步批注列表`);
      } catch {
  ...
  const sendFeedback = async () => {
    ...
    const nextRows = rows.map((item) => selectedRowKeys.includes(item.id)
      ? { ...item, feedbackTime: item.feedbackTime || sentAt, rounds: [...(item.rounds || []), { type: "send", role: "评估人员", content: "已向参股公司发送反馈，请补充回复材料。", time: sentAt }] }
      : item);
    persistRows(nextRows);
    setSubmitOpen(false);
    ...
    <div className={styles.page}>
      <div className={styles.tableCard}>
        <div className={styles.actionBar}>
          <span className={styles.summary}>共 {rows.length} 条需参股公司回复的批注，可导出给联系人并导入回答结果。</span>
          <Space>
            <Upload accept=".csv,.json" showUploadList={false} beforeUpload={beforeImport}>
              <Button icon={<UploadOutlined />}>导入Excel</Button>
            </Upload>
            <Button icon={<DownloadOutlined />} onClick={exportRows}>导出Excel</Button>
          </Space>
        </div>
              ...
              expandedRowRender: (record) => (
                <div className={styles.feedbackTimeline}>
                  {(record.rounds || []).map((round, index) => (
                    <div className={styles.feedbackTimelineItem} key={`${record.id}-${round.time}-${index}`}>
                      <span className={round.type === "reply" ? styles.feedbackTimelineReplyDot : styles.feedbackTimelineSendDot} />
                      <div>
        ...
        <div className={styles.footer}>
          <Button type="primary" disabled={!selectedRowKeys.length} onClick={() => setSubmitOpen(true)}>
            提交
          </Button>
        </div>
      </div>
      ...
      >
        <div className={styles.companyFeedbackModal}>
                ...
                children: (
                  <div className={styles.feedbackTopicPanel}>
                    <div className={styles.feedbackTopicHead}>
                      ...
                      <Tag color="processing">{topicGroup.pdfGroups.length}个PDF</Tag>
                    </div>
                    <Tabs
                      ...
                      items={topicGroup.pdfGroups.map((group, index) => ({
                        ...
                        children: (
                          <div className={styles.feedbackPdfPanel}>
                            ...
                            <iframe className={styles.feedbackPdfFrame} title={group.pdfName} src={group.pdfUrl} />
                            <div className={styles.feedbackPdfNotes}>
                              {group.rows.map((item) => (
                                <div className={styles.feedbackPdfNote} key={item.id}>
          ...
          <Form
            ...
            initialValues={{ contactName: "李明", email: "liming@example.com", phone: "13800000000" }}
          >
            <Form.Item label="联系人姓名" name="contactName" rules={[{ required: true, message: "请输入联系人姓名" }]}>
              <Input placeholder="请输入联系人姓名" />
            </Form.Item>
            <Form.Item label="邮件" name="email" rules={[{ required: true, message: "请输入邮件" }, { type: "email", message: "邮件格式不正确" }]}>
