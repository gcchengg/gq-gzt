...
export default function EvaluationDetail({
  ...
  const save = () =>
    message.success(`已保存“${topic?.topicName || "议题"}”评估详情`);
  const finish = () =>
    message.success(`已完成“${topic?.topicName || "议题"}”评估`);
  const openPdfEditor = (fileName, mode = "annotation") =>
    setPdfEditor({ fileName, mode });
  ...
  const opinionPane = (
    <div className={styles.gztOpinionLayout}>
      ...
      <section className={styles.gztOpinionFormCard}>
        <div
          className={`${styles.gztOpinionSectionTitle} ${styles.gztOpinionSummaryTitle}`}
        >
          综合意见
        </div>
        <div className={styles.gztOpinionForm}>
          <div
            className={`${styles.requiredLabel} ${styles.requiredHelpLabel}`}
          >
            综合意见
          ...
          <Radio.Group
            ...
            onChange={(event) => setOverallOpinion(event.target.value)}
          >
          ...
          <div className={styles.detailFooter}>
            {activeTab === "execute" ? (
              <>
                ...
                <Button type="primary" onClick={() => setActiveTab("opinion")}>
                  下一步
                </Button>
              </>
            ) : (
