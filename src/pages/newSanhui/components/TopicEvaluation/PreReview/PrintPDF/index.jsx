  ...
  ["factorLv3Name", "factorLv2Name", "assessElement"].forEach((field) => {
    ...
    for (let index = 0; index < processedData.length; index += 1) {
      ...
      if (currentValue !== prevValue) {
        ...
        for (let nextIndex = index + 1; nextIndex < processedData.length; nextIndex += 1) {
          const nextValue = processedData[nextIndex]?.sanhuiTopicModelFactorVo?.[field] || "";
        ...
        for (let nextIndex = index + 1; nextIndex < index + rowSpan; nextIndex += 1) {
          processedData[nextIndex].mergeInfo[field] = { rowSpan: 1, isShow: false };
        }
      }
    }
  });
...
const HtmlCell = ({ html }) => <div dangerouslySetInnerHTML={{ __html: html || "" }} />;

export default function PrintPDF({ infoData = {}, tableData = [] }) {
  const [fileList] = useState(defaultFileList);
  const processedTableData = useMemo(() => preprocessTableData(tableData), [tableData]);
  ...
  const pageStyles = {
    fontFamily: "\"阿里巴巴普惠体 3.0\", \"KaiLight\", \"deja-sans\", \"cjk\", sans-serif",
    fontSize: "16px",
    lineHeight: 1.6,
    color: "#212529",
    backgroundColor: "#fff",
  ...
  const columns = [
    ...
    {
      title: "评价标准",
      render: (_, record) => <HtmlCell html={record.sanhuiTopicModelFactorVo?.criterion} />,
    },
    {
      title: "执行情况",
    ...
    {
      title: "评价规则",
      render: (_, record) => <HtmlCell html={record.sanhuiTopicModelFactorVo?.assessRule} />,
    },
    {
      title: "评价结果(分)",
      render: (_, record) => {
        ...
        return record.sanhuiTopicModelFactorVo?.excludeAble === "1" && record.excludeFlag === "1"
          ? "/"
          : record.assessResult;
      },
    },
    {
      ...
      render: (_, record) => {
        ...
        if (record.sanhuiTopicModelFactorVo?.excludeAble === "1" && record.excludeFlag === "1") {
          colorKey = "green2";
        }
        return <div style={{ ...styles.circle, ...(styles[colorKey] || styles.green2) }} />;
      },
    },
  ];

  const hasDisqualified = processedTableData.some(
    (item) => item.sanhuiTopicModelFactorVo?.factorType === "2" && Number(item.assessResult) === 0,
  );
  const hasValidFactors = processedTableData.some(
    (item) => item.sanhuiTopicModelFactorVo?.factorType === "1",
  );

  const total = !hasDisqualified && hasValidFactors
    ? Number(
      processedTableData
        .filter((item) => item.sanhuiTopicModelFactorVo?.factorType !== "2")
        .reduce((pre, cur) => (
          pre + (Number(cur.sanhuiTopicModelFactorVo?.weight || 0) * (cur.excludeFlag === "1" ? 0 : Number(cur.assessResult || 0))) / 100
        ), 0)
        .toFixed(2),
    )
    : 0;

  const finalResult = hasDisqualified ? "不通过" : !hasValidFactors ? "通过" : total;
    ...
    <div style={pageStyles}>
      ...
      <div>
        ...
        {fileList.length > 0 ? (
          ...
          <div style={{ width: "100%", margin: "20px auto", textAlign: "center" }}>暂无图片</div>
        )}
          ...
          <table style={tableStyles}>
            ...
            <tbody>
              {processedTableData.map((item, index) => (
                <tr key={item.id || index} style={index % 2 === 0 ? styles.evenRow : null}>
                  <td style={{ ...tdStyles, textAlign: "center", verticalAlign: "middle" }}>
                    {index + 1}
                  </td>

                  {item.mergeInfo?.factorLv3Name?.isShow ? (
                    <td
                      style={{ ...tdStyles, textAlign: "center", verticalAlign: "middle" }}
                  ...
                  {item.mergeInfo?.assessElement?.isShow ? (
                    <td
                      style={{ ...tdStyles, textAlign: "left", paddingLeft: "8px" }}
                  ...
                  <td style={{ ...tdStyles, ...textCellStyles }}>{columns[5].render(null, item)}</td>
                  <td style={{ ...tdStyles, ...textCellStyles }}>{columns[6].render(null, item)}</td>
                  <td style={{ ...tdStyles, ...textCellStyles }}>{columns[7].render(null, item)}</td>
                  <td style={{ ...tdStyles, textAlign: "center", verticalAlign: "middle" }}>
              ...
              <tr>
                <td style={{ ...tdStyles, textAlign: "center", verticalAlign: "middle" }} colSpan={3}>
                  综合评价得分
                </td>
                <td style={{ ...tdStyles, fontSize: "0.9em", textAlign: "left", paddingLeft: "8px" }} colSpan={5}>
