...
const initialJointReviewRows = [
  {
    ...
    reviewer: "刘红艳",
    reviewerStatus: "done",
    reviewerFeedback: "本次事项预计不会突破年度预算控制线，付款节奏与现金流安排基本匹配。建议补充未来三年收益测算表，并说明敏感性测算假设来源。",
    managerTaskName: "财务科室经理审核确认",
    manager: "王经理",
    managerStatus: "pending",
    managerFeedback: "测算口径需统一为税后口径，付款节点建议与协议生效条件绑定。请将收益测算底稿、资金计划表作为附件随议案同步提交。",
  ...
  },
  {
    ...
    reviewer: "李娜",
    reviewerStatus: "done",
    reviewerFeedback: "议案审议路径基本符合制度要求，未发现明显程序瑕疵。合同文本中违约责任、争议解决条款仍偏原则，建议明确赔偿上限及退出触发条件。",
    managerTaskName: "审计风控与法务科室经理审核确认",
    manager: "赵经理",
    managerStatus: "pending",
    managerFeedback: "请在表决建议中单列法律风险提示，重点说明交易对手履约能力、担保措施有效性及历史遗留事项处理安排。",
    needManagerConfirm: true,
  },
  {
    id: "investment",
    ...
    reviewer: "陈浩",
    reviewerStatus: "done",
    reviewerFeedback: "项目商业逻辑较清晰，退出路径与当前市场窗口基本匹配。建议补充同类项目估值对比，并在后续执行中按月跟踪关键里程碑。",
    managerTaskName: "",
    manager: "",
    managerStatus: "",
    managerFeedback: "",
  ...
  },
  {
    ...
    reviewerFeedback: "该事项暂不涉及干部任免、机构调整等党委前置研究事项。建议在议案说明中补充“不涉及党委会前置研究”的判断依据，便于留痕备查。",
    managerTaskName: "",
    manager: "",
    managerStatus: "",
    managerFeedback: "",
  ...
  },
  {
    ...
    reviewer: "周敏",
    reviewerStatus: "pending",
    reviewerFeedback: "会议材料格式基本符合三会管理要求。建议统一附件命名规则，补齐签批页、议程版本号和材料提交时间，避免会前分发口径不一致。",
    managerTaskName: "",
    manager: "",
    managerStatus: "",
    managerFeedback: "",
...
export default function JointReviewFeedback({ isEdit = false }) {
  ...
  const columns = [
    ...
    },
    {
      ...
      width: 140,
      render: (value, record) => (
        record.needManagerConfirm ? <StatusTag status={value || "pending"} /> : <span>不涉及</span>
      ),
    },
    {
      title: "科室经理反馈建议",
