...
let followList = clone(initFollowSanhuiMgmtIdResponse.data?.sanhuiFollowVoList || []);

const ok = (data, message = "success") => ({ code: 200, data: clone(data), message });

export async function topicReportGet(params = {}) {
  await sleep();
  return ok({ ...reportData, sanhuiMgmtId: params.sanhuiMgmtId || reportData.sanhuiMgmtId });
}
...
export async function groupMessageDlvySave(params = {}) {
  ...
  messageDlvyData = {
    ...messageDlvyData,
    ...params,
    messageDlvyId: params.messageDlvyId || messageDlvyData.messageDlvyId || groupMessageDlvySaveResponse.data.messageDlvyId,
  };
  return { ...clone(groupMessageDlvySaveResponse), data: clone(messageDlvyData) };
}

export async function groupMessageDlvySubmit(params = {}) {
  await sleep();
  messageDlvyData = { ...messageDlvyData, ...params };
  return { ...clone(groupMessageDlvySubmitResponse), data: clone(messageDlvyData) };
}
...
export async function topicReportSave(params = {}) {
  ...
  return {
    ...
    message: params.status === "1" ? "提交成功" : topicReportSaveResponse.message,
  };
}
...
export async function getFollowInfo(params = {}) {
  await sleep();
  return ok(followList.find((item) => item.id === params.id) || getFollowInfoResponse.data);
}
