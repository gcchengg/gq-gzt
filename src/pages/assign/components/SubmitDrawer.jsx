import { Button, Card, Checkbox, DatePicker, Form, Input, Radio, Select, Space, Switch, Table, Tabs, Tag, Upload, message, } from "antd";
...
function SmartSubmit({ disabled, onNext, }) {
    ...
    const columns = [
        ...
        },
        {
            title: "操作",
            width: 90,
            render: (_value, record) => disabled ? null : (<Button type="link" danger onClick={() => setFiles((current) => current.filter((item) => item.id !== record.id))}>
            删除
          </Button>),
        },
    ];
    return (<div className="submit-section">
      <Prompt type="smart"/>
      <Upload.Dragger disabled={disabled} multiple={false} showUploadList={false} beforeUpload={(file) => {
            const newFile = {
                id: `smart-${Date.now()}`,
                fileName: file.name,
...
function TopicManage({ disabled, onPrev, onNext, }) {
    ...
    const columns = [
        ...
        { title: "审批层级", dataIndex: "reviewLevel2" },
        {
          ...
          <Button type="link" onClick={() => message.info(`${record.toipcName} 使用假数据详情`)}>
            编辑
          </Button>
          {!disabled ? (<Button type="link" danger onClick={() => setTopics((current) => current.filter((item) => item.id !== record.id))}>
              删除
            </Button>) : null}
        </Space>),
        },
    ...
    return (<div className="submit-section">
      ...
      <Form form={form} layout="vertical">
        <div className="submit-filter-grid">
          <Form.Item name="categoryLv1Name" label="议题分类（大）">
            <Select allowClear options={categoryOptions.lv1.map((item) => ({ ...item, value: item.label }))}/>
          </Form.Item>
          <Form.Item name="categoryLv2Name" label="议题分类（中）">
            <Select allowClear options={categoryOptions.lv2.map((item) => ({ ...item, value: item.label }))}/>
          </Form.Item>
          <Form.Item name="categoryLv3Name" label="议题分类（小）">
            <Select allowClear options={categoryOptions.lv3.map((item) => ({ ...item, value: item.label }))}/>
          </Form.Item>
          <Form.Item name="reviewLevel2" label="审批层级">
            <Select allowClear options={reviewLevelOptions.map((item) => ({ label: item.label, value: item.label }))}/>
          </Form.Item>
      ...
      <Table rowKey="id" columns={columns} dataSource={topics} pagination={false} size="small"/>
      {!disabled ? (<div className="submit-footer">
    ...
    return (<div className="submit-section">
      ...
      <div className="distribution-layout">
        <div className="distribution-left">
          ...
          <Table rowKey="id" columns={columns} dataSource={data} pagination={false} size="small" scroll={{ x: 1260 }}/>
        </div>
