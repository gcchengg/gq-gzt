...
const defaultDecisionExecList = [
  {
    ...
    topicType: "1",
    gqPassFlag: "1",
    investmentBodPassFlag: "1",
    investmentBosPassFlag: "1",
    investmentShPassFlag: "1",
    bodPassFlag: "1",
    bosPassFlag: "1",
    shPassFlag: "1",
    diffRemark: "",
  },
  {
    ...
    topicType: "1",
    gqPassFlag: "1",
    investmentBodPassFlag: "2",
    investmentBosPassFlag: "2",
    investmentShPassFlag: "2",
    bodPassFlag: "2",
    bosPassFlag: "2",
    shPassFlag: "2",
    diffRemark:
      "三会审议时增加了预算执行过程中的月度监测条件，需按有条件通过跟踪。",
  },
  {
...
const getStatusText = (status) =>
  sanhuiProgStatus.find((item) => String(item.value) === String(status))
    ?.text || "-";
...
export async function threeListGetList(params = {}) {
  ...
  const filtered = records.filter((record) => {
    const codeMatched = params.companyCreditCode
      ? String(record.companyCreditCode || "").includes(
          params.companyCreditCode,
        )
      : true;
...
export async function saveExec(params = {}) {
  ...
  if (params.sanhuiMgmtId && params.sanhuiVoteTopicList) {
    decisionExecByMgmtId[params.sanhuiMgmtId] = clone(
      params.sanhuiVoteTopicList,
    );
  }
  return {
    code: 200,
...
export async function saveFollow(record = {}) {
  ...
  if (!savedRecord) {
    savedRecord = {
      ...
      deadlineDate:
        record.deadlineDate || dayjs().add(10, "day").format("YYYY-MM-DD"),
...
export async function createAssignFollowTask(params = {}) {
  ...
  const task = {
    ...
    attachmentName:
      firstFollow.aiSourceFileName || "截屏2026-04-15 17.55.31.png",
    taskType: "",
...
export async function getAssignFollowTask(params = {}) {
  ...
  const task =
    assignFollowTaskById[taskId] ||
    assignFollowTaskById[latestAssignFollowTaskId];
  return {
    code: 200,
    data: clone(task),
    message: "success",
