import { Button, Input, Spin, Table, Tabs, Tag, Tooltip, message } from "antd";
import { QuestionCircleOutlined } from "@ant-design/icons";
import { useEffect, useMemo, useState } from "react";
import { initDecisionExec, saveExec } from "../mockApi";
import AssignFollow from "./AssignFollow";
import "./DecisionExecution.css";

const hasDecisionValue = (value) =>
...
const getOfficeDecision = (row, meetingKey) => {
  ...
  if (
    hasDecisionValue(row.gqPassFlag) &&
    hasDecisionValue(getThreeDecision(row, meetingKey))
  )
    return row.gqPassFlag;
  return "";
};
...
const isInvestmentTopic = (row) =>
  row.topicType === "1" || row.eoSanhuiTopic?.topicType === "1";

const getInvestmentDecision = (row, meetingKey) => {
  if (!isInvestmentTopic(row)) return null;
  const fieldMap = {
    bod: "investmentBodPassFlag",
    bos: "investmentBosPassFlag",
    sh: "investmentShPassFlag",
  };
  return row[fieldMap[meetingKey]];
};

const renderInvestmentDecision = (row, meetingKey) => {
  const value = getInvestmentDecision(row, meetingKey);
  if (value === null) return "--";
  return passText(value);
};
...
const getDecisionConsistency = (row) => {
  ...
  const compared = meetingKeys
    ...
    .filter(
      ({ officeValue, threeValue }) =>
        hasDecisionValue(officeValue) || hasDecisionValue(threeValue),
    );
  ...
  return compared.every(
    ({ officeValue, threeValue }) =>
      String(officeValue || "") === String(threeValue || ""),
  );
};
  ...
  const decisionColumns = useMemo(
    () => [
      ...
      },
      {
        ...
        width: 260,
        render: (_value, row) => (
          <span className="assign-decision-topic-name">
            <span>
              {row?.eoSanhuiTopic?.toipcName || row?.toipcName || "-"}
            </span>
            {row.topicType === "1" || row.eoSanhuiTopic?.topicType === "1" ? (
              <Tag color="gold">股权投委会</Tag>
            ) : null}
          </span>
        ),
      },
      ...
      },
      {
        ...
        children: [
          {
            title: "董事会",
            dataIndex: "investmentBodPassFlag",
            align: "center",
            width: 120,
            render: (_value, row) => renderInvestmentDecision(row, "bod"),
          },
          {
            title: "监事会",
            dataIndex: "investmentBosPassFlag",
            align: "center",
            width: 120,
            render: (_value, row) => renderInvestmentDecision(row, "bos"),
          },
          {
            title: "股东会 / 投委会",
            dataIndex: "investmentShPassFlag",
            align: "center",
            width: 150,
            render: (_value, row) => renderInvestmentDecision(row, "sh"),
          },
        ],
      },
      {
  ...
  const decisionTab = (
    <div className="assign-decision">
      <div className="assign-warning">
        <strong>决策情况</strong>
        <span>以下列表中总办会与三会决议不一致时，请填写原因说明。</span>
      </div>
      <Table
              ...
              label: (
                <span className="assign-tab-label-with-help">
                  ...
                  <Tooltip
                    title={
                      <div>
                        <div>1.选项统一 同意 反对 有条件同意 回避表决</div>
                        <div>2. 股东会议决策 数据 取自审批材料准备</div>
                      </div>
                    }
                  >
                    <QuestionCircleOutlined
                      className="assign-tab-help-icon"
