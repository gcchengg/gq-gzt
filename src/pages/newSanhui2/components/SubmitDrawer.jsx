...
function TopicEditDrawer({ open, mode, record, disabled, onClose, onSave }) {
  ...
  return (
    <Drawer
      title={
        <span className="topic-edit-drawer-title">
          {drawerTitle}
          <Tooltip title="1.删除回避表决">
            <QuestionCircleOutlined className="topic-edit-title-help" />
          </Tooltip>
        </span>
      }
      width={960}
      ...
      onClose={onClose}
      destroyOnClose
      className="topic-edit-drawer-shell"
      footer={
        <div className="topic-edit-drawer-footer">
      ...
      <div className="topic-edit-drawer">
        ...
        >
          <div className="topic-edit-hero">
            <div>
              <div className="topic-edit-hero-title">
                {mode === "edit" ? "维护议题信息" : "创建一条新议题"}
              </div>
              <div className="topic-edit-hero-desc">
                补全议题基础信息、参会范围和审批分类后，可继续生成会议安排。
              </div>
            </div>
          ...
          <div className="topic-edit-block">
            ...
            <div className="topic-edit-grid">
              ...
              <Form.Item
                label="是否为股权投委会"
                name="topicType"
                rules={[{ required: true, message: "请选择是否为股权投委会" }]}
              >
                <Radio.Group
                  options={[
                    { label: "是", value: "1" },
...
function SmartSubmit({ disabled, onNext }) {
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (_value, record, index) =>
        disabled ? null : (
          <Space className="submit-table-action" size={4}>
            <Button
              type="link"
              disabled={index === 0}
  ...
  return (
    <div className="submit-section">
      ...
      <Panel title="提报材料">
        <Upload.Dragger
          className="submit-upload-dragger"
          disabled={disabled}
          multiple={false}
...
function TopicManage({ disabled, onPrev, onNext }) {
  ...
  const moveTopic = (index, offset) => {
    setTopics((current) => {
      ...
      setAllTopics((allCurrent) => {
        const targetIndex = allCurrent.findIndex(
          (item) => item.id === target.id,
        );
        const neighborIndex = allCurrent.findIndex(
        ...
        [allNext[targetIndex], allNext[neighborIndex]] = [
          allNext[neighborIndex],
          allNext[targetIndex],
        ];
        return allNext;
      });
  ...
  const columns = [
    ...
    { title: "审批层级", dataIndex: "reviewLevel2", width: 110 },
    {
      ...
      fixed: "right",
      render: (_value, record, index) => (
        <Space className="submit-topic-actions" size={4}>
          <Button
            type="link"
            size="small"
            onClick={() => {
    ...
    <div className="submit-section">
      ...
      <div className="distribution-layout">
        <Panel
          title="议题材料传达对象"
          extra={
            <span className="distribution-notice danger">
              <span className="distribution-notice-icon">!</span>
              <span>集团总经理助理及以上不传达</span>
            </span>
          }
        >
        ...
        <Panel
          title="职能联审议题材料传达对象"
          extra={
            <span className="distribution-notice warning">
              <span className="distribution-notice-icon">!</span>
              <span>指定参与职能联审初审的职能部门</span>
            </span>
          }
        >
      ...
      {!disabled ? (
        <div className="submit-footer">
          <span className="submit-footer-hint">
            提交后将同时发起议题初审、议题评估、董事意见反馈与表决建议单任务
          </span>
          <Button onClick={onPrev}>上一步</Button>
    ...
    () => [
      {
        ...
        label: (
          <span className="submit-tab-label-with-help">
            智能提报
            <Tooltip
              title={smartSubmitTip}
              placement="topLeft"
              overlayClassName="submit-distribution-tooltip"
            >
              <QuestionCircleOutlined
                className="submit-tab-help-icon"
      ...
      },
      {
        ...
        children: (
          <MeetingManage
            ...
            confirmEnabledChange={canEditMeetingInEvaluation}
            showFooter={!disabled}
            onPrev={goPrev}
            onNext={goNext}
          />
