...
const seedMaterials = [
  {
    id: "1",
    name: "20250428中联电子议题关键信息页(1)P1.jpg",
    source: "批注内容",
    selected: true,
  },
  {
    id: "2",
    name: "董事会议案表决建议P2.jpg",
    source: "批注内容",
    selected: true,
  },
  {
    id: "3",
    name: "20250428中联电子议题关键信息页(1)P3.jpg",
    source: "手动上传",
    selected: false,
  },
  {
    id: "4",
    name: "20250428中联电子议题关键信息页(1)P4.jpg",
    source: "手动上传",
    selected: false,
  },
];
...
export function SupplementMaterials({ open, onClose }) {
  ...
  return (
    ...
    >
      <div className={styles.materialSteps}>
        ⚠ 第一步：可以混合上传多张图片和 PDF 文件，PDF
        会按打印分页自动拆解成图片。
        <br />⚠ 第二步：通过列表第一列选择本议题展示的材料图片。
      </div>
      ...
      <Table
        ...
        columns={[
          ...
          },
          {
            ...
            width: 120,
            render: (value) => (
              <Tag color={value === "批注内容" ? "purple" : "blue"}>
                {value}
              </Tag>
            ),
          },
...
export function EvaluationPreview({ open, topic, onClose }) {
  const previewTopic = useMemo(() => {
    const topicName =
      topic?.topicName || topic?.toipcName || topic?.proposalName;
    const assessmentList = previewData?.data?.assessmentList || [];
    return (
      assessmentList.find((item) => item.topicName === topicName) ||
      assessmentList[0] ||
      {}
    );
  }, [topic]);
