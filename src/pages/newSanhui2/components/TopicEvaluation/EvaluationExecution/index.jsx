...
function readScreenshotPages() {
  ...
  try {
    return (
      JSON.parse(window.localStorage.getItem(screenshotStorageKey)) || fallback
    );
  } catch {
    return fallback;
  }
...
export default function EvaluationExecution({ onOpenPdf }) {
  ...
  useEffect(() => {
    ...
    window.addEventListener(
      "newSanhui:annotationScreenshotPagesChange",
      handleChange,
    );
        ...
        <div className="gzt-eval-section-head">
          ...
          <div className="gzt-eval-section-actions">
            <Upload
              ...
              beforeUpload={addSupplementAttachment}
            >
                    ...
                    <div className="gzt-eval-snapshot-grid">
                      {(file.snapshots || []).length ? (
                        ...
                        <div className="gzt-eval-snapshot-empty">
                          当前文件暂无收藏过的批注页截图，请进入编辑 PDF
                          抽屉，将需要关联的页面加入汇报材料页收藏。
                        </div>
                      )}
                    </div>
                  ),
