      ...
      topics: [
        {
          ...
          name: "关于推进基金退出事项的议案",
          files: [
            { name: "基金退出方案补充材料.pdf", url: feedbackReferenceUrl },
          ],
          records: [
            {
              id: "feedback-001",
              ...
              content:
                "请补充基金退出方案中交易对手资信情况，以及本次退出对年度收益目标的影响测算。",
            },
            {
              id: "feedback-002",
              role: "manager",
              ...
              content: "已收到，管户团队正在补充资信核查表和收益测算口径。",
            },
          ],
        },
        {
          ...
          files: [
            { name: "外部董事意见采纳情况说明.pdf", url: feedbackReferenceUrl },
          ],
          records: [
            {
              id: "feedback-003",
              ...
              content: "请逐项标注外部董事意见是否采纳以及未采纳原因。",
            },
          ],
        },
      ],
          ...
          records: [
            {
              ...
              content:
                "请同步说明是否涉及其他股东优先购买权，以及法律合规部是否已出具书面意见。",
            },
            {
              id: "feedback-005",
              role: "manager",
              ...
              content: "已核对章程及投资协议，不触发其他股东优先购买权。",
            },
          ],
        },
      ],
      ...
      topics: [
        {
          ...
          name: "关于推进基金退出事项的议案",
          files: [
            { name: "表决建议单-基金退出.pdf", url: feedbackReferenceUrl },
          ],
          records: [
            {
              ...
              content:
                "表决建议中请明确本次基金退出的表决倾向，并补充收益测算依据和风险兜底安排。",
            },
            {
      ...
      topics: [
        {
          ...
          name: "关于补充外部董事意见采纳情况的议案",
          files: [
            { name: "表决建议单-董事意见采纳.pdf", url: feedbackReferenceUrl },
          ],
          records: [
            {
              ...
              content:
                "请在表决建议单中说明是否需要附带授权条件，避免后续执行口径不一致。",
            },
            {
              ...
              content: "已补充授权条件，协议签署文本需经法务复核后方可执行。",
            },
          ],
        },
      ],
    ...
    <section className="assign-feedback-topic">
      ...
      <div className="assign-feedback-chat-body">
        {topic.records.map((item) => (
          <div
            className={`assign-feedback-row ${item.role === "manager" ? "is-manager" : "is-leader"}`}
            key={item.id}
          >
...
function DirectorFeedbackPanel({
  type,
  directors,
  drafts,
  onDraftChange,
...
function TabLabel({ active, disabled, stageKey, children }) {
  ...
  const reviewRemovedTip =
    stageKey === "3" ? (
      <div className="new-sanhui-review-remove-tip">
        <div className="new-sanhui-review-remove-title">
          Demo说明：已删除子tab【议题初审问答】
        </div>
        <img
          src="/new-sanhui-review-removed-question-tab.png"
          alt="已删除的议题初审问答子tab截图"
        />
        <div className="new-sanhui-review-remove-title">
          Demo说明：已删除文件替换tab页面
        </div>
        <img
          src={removedFileReplaceTabImage}
          alt="已删除的文件替换tab页面截图"
        />
      </div>
    ) : null;
        ...
        <strong>
          ...
          {removedTip ? (
            <Tooltip
              placement="bottom"
              title={removedTip}
              overlayClassName="new-sanhui-review-remove-tooltip"
            >
              <QuestionCircleOutlined
...
const voteAuthorizeMeetingMap = {
  MEETING_DS: {
    fileKey: "DSfileList",
    meetingKey: "meetingDs",
    disabledKey: "disabledDs",
  },
  MEETING_JS: {
    fileKey: "JSfileList",
    meetingKey: "meetingJs",
    disabledKey: "disabledJs",
  },
  MEETING_GD: {
    fileKey: "GDfileList",
    meetingKey: "meetingGd",
    disabledKey: "disabledGd",
  },
};
...
function VoteAuthorizeDrawer({ open, onClose }) {
  ...
  const updateRow = (rowKey, patch) => {
    setDataSource((current) =>
      current.map((item) =>
        item.key === rowKey ? { ...item, ...patch } : item,
      ),
    );
  };
        ...
        render: (value = [], row) => (
          <VoteAuthorizeUpload
            ...
            onChange={(nextFileList) =>
              updateRow(row.key, { [config.fileKey]: nextFileList })
            }
          />
        ),
      },
...
const initialMeetingTimeList = meetingGetListResponse.data
  .filter((item) =>
    ["董事会", "监事会", "股东会"].includes(item.meetingTypeName),
  )
  .map((item, index) => ({
    ...item,
  ...
  return (
    <div className="meeting-card">
      <div className="meeting-card-head">
        ...
        <Space>
          <Switch checked={meeting.enabled} onChange={handleEnabledChange} />
          <span>召开</span>
        </Space>
      </div>
      <Form
        ...
        {steps.map((step, index, list) => (
          ...
          >
            <div className="meeting-time-approval-rail">
              <span className="meeting-time-approval-dot">
                {step.state === "approved" ? "✓" : ""}
              </span>
              {index < list.length - 1 ? (
                <span className="meeting-time-approval-line" />
              ) : null}
            </div>
            <div className="meeting-time-approval-card">
              <div className="meeting-time-approval-card-head">
                ...
                {step.state === "approved" ? (
                  <span className="meeting-time-approval-status">审批通过</span>
                ) : null}
                {step.state === "pending" ? (
                  <span className="meeting-time-approval-status is-pending">
                    待审批
                  </span>
                ) : null}
              </div>
...
function MeetingTimeManage({ onSave }) {
  ...
  const confirmPendingEnabledChange = () => {
    if (pendingEnabledChange) {
      updateMeeting({
        ...pendingEnabledChange.meeting,
        enabled: pendingEnabledChange.checked,
      });
    }
          ...
          <div className="meeting-grid">
            {meetings.map((meeting) => (
              <MeetingTimeCard
                key={meeting.key}
                meeting={meeting}
                onEnabledChange={(nextMeeting, checked) =>
                  setPendingEnabledChange({ meeting: nextMeeting, checked })
                }
  ...
  return (
    <div className="meeting-minutes-wrap">
      ...
      <Upload
        ...
        maxCount={1}
      >
        <Button disabled={!canEdit} icon={<UploadOutlined />}>
          上传会议纪要
        </Button>
      </Upload>
...
function EquityInvestmentMeeting({ canEdit = false }) {
  ...
  return (
    <div className="meeting-minutes-wrap">
      <div className="meeting-minutes-title">股权投委会会议纪要</div>
      <Form
        form={form}
        layout="vertical"
        disabled={!canEdit}
        initialValues={{
          investmentLaunchDate: dayjs("2026-04-29"),
          investmentIssueNo: 1,
        }}
      >
        <div className="meeting-minutes-form-grid">
          <Form.Item
            label="股权投委会召开日"
            name="investmentLaunchDate"
            rules={[{ required: true, message: "请选择日期" }]}
          >
            <DatePicker format="YYYY-MM-DD" style={{ width: "100%" }} />
          </Form.Item>
          <Form.Item label="期数" name="investmentIssueNo">
            <InputNumber
              min={1}
              precision={0}
              placeholder="请输入期数"
  ...
  useImperativeHandle(ref, () => ({
    getSaveData: () => {
      ...
      const findLabel = (value) =>
        meetingUserOptions.find((item) => item.value === value)?.label || "";
      const mapUsers = (list = [], userType) =>
        list
          .map((value) => ({
            userType,
            userId: value,
            userName: findLabel(value),
          }))
      ...
      return {
        ...
        launchDate: values.launchDate
          ? dayjs(values.launchDate).format("YYYY-MM-DD")
          : null,
  ...
  const updateTopic = (recordKey, field, value, recordIndex) => {
    setTopicList((list) =>
      list.map((item, index) => {
        ...
        return key === recordKey || index === recordIndex
          ? { ...item, [field]: value }
          : item;
      }),
    );
  ...
  const columns = [
    ...
    { title: "议案名称", dataIndex: "topicName", width: 240 },
    {
      ...
      render: (_value, record, index) =>
        renderResultSelect(record, "bodCond", index),
    },
    {
      title: "监事会审议",
      dataIndex: "bosCond",
      width: 160,
      render: (_value, record, index) =>
        renderResultSelect(record, "bosCond", index),
    },
    {
      title: "股东会审议",
      dataIndex: "shCond",
      width: 160,
      render: (_value, record, index) =>
        renderResultSelect(record, "shCond", index),
    },
  ];
      ...
      <Form layout="vertical" form={form}>
        <div className="meeting-minutes-form-grid">
          ...
          <Form.Item
            label="出席"
            name="cxrList"
            className="meeting-minutes-full-row"
          >
            <Select
              mode="multiple"
              options={meetingUserOptions}
        ...
        <div className="meeting-minutes-table-section">
          <div className="meeting-minutes-section-title">
            议题主要内容及讨论意见
          </div>
          <Table
...
export default function DueDrawer({
  ...
  const [reviewActiveKey, setReviewActiveKey] = useState(
    reviewInitialTab || "1",
  );
  ...
  const currentStepIndex = Math.max(
    0,
    phaseSteps.findIndex((item) => item.value === progressPhaseKey),
  );
  const progressPercent =
    phaseSteps.length > 1
      ? Math.round((currentStepIndex / (phaseSteps.length - 1)) * 100)
      : 0;
  ...
  const showMeetingSave =
    canEditMeetingMinutes &&
    (meetingActiveKey === "1" ||
      meetingActiveKey === "investment" ||
      meetingActiveKey === "2" ||
      meetingActiveKey === "3");
    ...
    () => [
      {
        ...
        children: (
          <SubmitDrawer
            ...
            editStatus={
              editStatus === "edit" &&
              (progStatus === "12000" || progStatus === "13000")
                ? "edit"
                : "detail"
            }
            ifInitSubmit={false}
      ...
      },
      {
        ...
        disabled: isTabDisabled("2"),
        children: (
          <TopicEvaluation projectData={projectData} onClose={onCloseDetail} />
        ),
      },
      {
        label: (
          <TabLabel
            active={firstActiveKey === "3"}
      ...
      },
      {
        ...
        children: (
          <VoteSuggest
            id={id}
            editStatus={
              editStatus === "edit" && progStatus === "16000"
                ? "edit"
                : "detail"
            }
          />
      ...
      },
      {
        ...
        children: (
          <Vote
            ...
            editStatus={
              editStatus === "edit" && progStatus === "18000"
                ? "edit"
                : "detail"
            }
          />
      ...
      },
      {
            ...
            <span className="new-sanhui-tab-title-with-help">
              ...
              <Tooltip
                title={
                  <div>
                    <div>
                      1.删除 董事会会议决议/监事会会议决议/股东会会议决议
                      完整版文件上传功能
                    </div>
                    <div>2.删除 会议纪要上传功能</div>
                  </div>
                }
              >
                <QuestionCircleOutlined
        ...
        children: (
          <DecisionExecution
            ...
            editStatus={
              editStatus === "edit" &&
              (progStatus === "19000" || progStatus === "20000")
            }
  ...
  const handleMeetingSave = () => {
    ...
    if (meetingActiveKey === "investment") {
      message.success("股权投委会会议纪要信息已保存");
      return;
    }
    const ref =
      meetingActiveKey === "2"
        ? viceMeetingRef.current
        : meetingActiveKey === "3"
          ? directorMeetingRef.current
          : null;
  ...
  const sendFeedbackReply = (type, directorKey) => {
    const director = feedbackGroups[type].find(
      (item) => item.key === directorKey,
    );
    const replies =
      director?.topics
        .map((topic) => {
          ...
          return {
            topicId: topic.id,
            draftKey,
            content: feedbackDrafts[draftKey]?.trim(),
          };
        })
    ...
    setFeedbackGroups((current) => ({
      ...current,
      [type]: current[type].map((directorItem) =>
        directorItem.key === directorKey
          ? {
              ...directorItem,
    ...
    <div className="new-sanhui-due">
      <div className="new-sanhui-due-left">
        ...
        <div className="new-sanhui-project-tabs">
          {hasVisibleActions && (
            <div className="new-sanhui-button-group">
              ...
              {actionVisibility.meetingTime && (
                <Button
                  type="primary"
                  onClick={() => setMeetingTimeDrawerOpen(true)}
                >
                  <span className="new-sanhui-action-title-with-help">
                    修改会议信息
                    <Tooltip
                      title={
                        <div>
                          <div>1.审批层级到 科室经理</div>
                          <div>2.只有修改会议时间的时候才发起审批</div>
                        </div>
                      }
                    >
                      <QuestionCircleOutlined
          ...
          <Tabs
            className="new-sanhui-stage-content-tabs"
          ...
          {voteRecords.length > 0 ? (
            voteRecords.map((item) => (
              <div className="new-sanhui-vote-item" key={item.id}>
                ...
                >
                  <Descriptions.Item label="投票人">
                    {item.userName}
                  </Descriptions.Item>
                  <Descriptions.Item label="计划投票日">
                    {item.voteTime
                      ? dayjs(item.voteTime).format("YYYY-MM-DD")
                      : ""}
                  </Descriptions.Item>
      ...
      >
        <Tabs
          activeKey={meetingActiveKey}
          onChange={(key) => setMeetingActiveKey(key)}
          items={meetingTabItems}
        />
      </Drawer>
