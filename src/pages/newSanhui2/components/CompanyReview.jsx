...
import {
  EvaluationPreview,
  SupplementMaterials,
} from "./TopicEvaluation/EvaluationMaterials";
...
const investmentAgendaDefault =
  "5.2.7公司投资事业部、公司股权运营部负责管理的参股企业拟提交股东会、董事会、监事会的部分议题（议题清单见附录1）；-----本次工作台只涉及这一项下的11类议题修改";
...
function ReviewHeader({ projectData }) {
  return (
    <Card className="review-summary" size="small">
...
function FilesReplacePanel({ isEdit }) {
  ...
  const openUploadConfirm = (fileList, record) => {
    ...
    const duplicated = selectedFiles.some((file) =>
      files.some(
        (item) => item.fileName === file.name && item.id !== record?.id,
      ),
    );
    setUploadConfirm({
  ...
  const handleUploadConfirm = () => {
    ...
    setTimeout(() => {
      if (uploadConfirm.type === "replace") {
        ...
        setFiles((current) =>
          current.map((item) =>
            item.id === uploadConfirm.record.id
              ? {
                  ...item,
                  fileName: file.name,
        ...
        setFiles((current) => {
          ...
          return [
            ...incoming,
            ...current.filter((item) => !incomingNames.has(item.fileName)),
          ];
        });
        message.success(
          uploadConfirm.duplicated ? "同名文件已替换" : "上传成功",
        );
      }
      setLoading(false);
      setUploadConfirm(null);
    ...
    { title: "上传时间", dataIndex: "created", width: 180 },
    {
      ...
      render: (_value, record) => (
        <Space>
          ...
          >
            <Button type="link" danger disabled={!isEdit}>
              删除
            </Button>
          </Popconfirm>
        </Space>
...
function QuestionsPanel({ isEdit, onCountChange }) {
  ...
  const selectedQuestion =
    questions.find((item) => item.id === selectedId) || questions[0];
  ...
  const endQuestion = () => {
    ...
    setQuestions((current) =>
      current.map((item) =>
        item.id === selectedQuestion.id ? { ...item, state: "2" } : item,
      ),
    );
    onCountChange(
      questions.filter(
        (item) => item.id !== selectedQuestion.id && item.state !== "2",
      ).length,
    );
    ...
    <div className="question-layout">
      ...
      <div className="question-right">
        <div className="question-detail-title">
          {selectedQuestion?.topicName || "问答详情"}
        </div>
        <div className="question-message-list">
          {selectedQuestion?.messages.map((item) => (
            <div className={`question-message ${item.type}`} key={item.id}>
        ...
        <Input.TextArea
          rows={4}
          value={answer}
          disabled={!isEdit || selectedQuestion?.state === "2"}
          placeholder="请输入提问或回答内容"
      ...
      <Modal
        ...
        onOk={() => {
          form.validateFields().then((values) => {
            ...
            onCountChange(
              questions.filter((item) => item.state !== "2").length + 1,
            );
            form.resetFields();
            setAddOpen(false);
          });
...
const approvalPreviewImages = [
  {
    id: "approval-preview-image-1",
    fileName: "1.png",
    fileUrl: approvalPreviewImage1,
  },
  {
    id: "approval-preview-image-2",
    fileName: "2.png",
    fileUrl: approvalPreviewImage2,
  },
];
...
function ApprovalPreviewModal({ open, onClose, reviewType }) {
  ...
  const calculateRowSpan = (data, field, index) => {
    if (
      index === 0 ||
      data[index]?.sanhuiTopicModelFactorVo?.[field] !==
        data[index - 1]?.sanhuiTopicModelFactorVo?.[field]
    ) {
      let count = 1;
      for (let i = index + 1; i < data.length; i += 1) {
        if (
          data[i]?.sanhuiTopicModelFactorVo?.[field] ===
          data[index]?.sanhuiTopicModelFactorVo?.[field]
        ) {
          count += 1;
        } else {
  ...
  const meetingInfoColumns = [
    ...
    {
      title: "会议日期",
      dataIndex: "launchTimeStr",
      width: "14%",
      align: "center",
    },
    {
      title: "会议形式",
      dataIndex: "launchTypeName",
      width: "14%",
      align: "center",
    },
  ];
  const topicInfoColumns = [
    ...
    { title: "议案名称", dataIndex: "toipcName", width: "52%" },
    {
      ...
      align: "center",
      render: (_text, record) => {
        const schedule = record.eoSanhuiTopicScheduleFormList?.find(
          (item) => item.meetingType === "100",
        );
        return schedule?.launchFlag === "1" ? (
          <div className="approval-preview-stack">
            <div>√</div>
            {schedule.voteElusionFlag === "1" ? <div>（回避表决）</div> : null}
          </div>
        ) : (
          "-"
        );
  ...
  const assessmentColumns = (data) => [
    ...
    },
    {
      ...
      render: (_text, record, index) => ({
        ...
        props: { rowSpan: calculateRowSpan(data, "factorLv3Name", index) },
      }),
    },
    {
      title: "二级维度",
      ...
      align: "center",
      render: (_text, record, index) => ({
        children: record.sanhuiTopicModelFactorVo?.factorLv2Name,
        props: { rowSpan: calculateRowSpan(data, "factorLv2Name", index) },
      }),
    },
    {
      title: "评价要素",
      ...
      align: "center",
      render: (_text, record, index) => ({
        children: record.sanhuiTopicModelFactorVo?.assessElement,
        props: { rowSpan: calculateRowSpan(data, "assessElement", index) },
      }),
    },
    {
      title: "权重",
    ...
    },
    {
      title: "评价标准",
      render: (_text, record) => (
        <HtmlText value={record.sanhuiTopicModelFactorVo?.criterion} />
      ),
    },
    {
      title: "执行情况",
      render: (_text, record) => <HtmlText value={record.execDetail} />,
    },
    {
      title: "评价规则",
      render: (_text, record) => (
        <HtmlText value={record.sanhuiTopicModelFactorVo?.assessRule} />
      ),
    },
    {
      title: "评价结果(分)",
    ...
    },
    {
      ...
      render: (_text, record) => (
        <span
          className={`approval-preview-circle ${record.excludeFlag === "1" ? "green2" : record.ligh || ""}`}
        />
      ),
    },
  ];
  ...
  const decisionColumns = [
    ...
    { title: "议案名称", dataIndex: "sanhuiTopicName", width: "46%" },
    {
      ...
      align: "center",
      render: (_text, record) =>
        record.voteAdvice100 ? (
          <div className="approval-preview-stack">
            <div>{voteAdviceTextMap[record.voteAdvice100]}</div>
            {record.mgmtComment100 ? (
              <div>（{record.mgmtComment100}）</div>
            ) : null}
          </div>
        ) : (
          "-"
    ...
    },
    {
      ...
      align: "center",
      render: (_text, record) =>
        record.voteAdvice200 ? (
          <div className="approval-preview-stack">
            <div>{voteAdviceTextMap[record.voteAdvice200]}</div>
            {record.mgmtComment200 ? (
              <div>（{record.mgmtComment200}）</div>
            ) : null}
          </div>
        ) : (
          "-"
    ...
    },
    {
      ...
      align: "center",
      render: (_text, record) =>
        record.voteAdvice300 ? (
          <div className="approval-preview-stack">
            <div>{voteAdviceTextMap[record.voteAdvice300]}</div>
            {record.mgmtComment300 ? (
              <div>（{record.mgmtComment300}）</div>
            ) : null}
          </div>
        ) : (
          "-"
  ...
  return (
    <Modal
      title={reviewTitleMap[reviewType]}
      open={open}
      width="100%"
      style={{ top: 0 }}
      className="search-modal approval-preview-modal"
      onCancel={onClose}
      footer={null}
    >
      <div className="approval-preview-doc">
        <h1 className="approval-preview-doc-title">
          <div className="approval-preview-main-title">
            <HtmlText value={infoData.companyName} />
          </div>
          <span className="approval-preview-dept-title">
            {infoData.deptName}
          </span>
        </h1>
        <h2>提报材料</h2>
        <Table
          rowKey="id"
          className="approval-preview-table"
          columns={[
            {
              title: "序号",
              width: "10%",
          ...
          dataSource={infoData.reportFileList || []}
          pagination={false}
          bordered
        />
        <h2>风控合规审核意见</h2>
        <p>
          <HtmlText value={infoData.lagelComment} />
        </p>
        <h2>风控合规风险提示应对建议</h2>
        <p>
          <HtmlText value={infoData.riskComment} />
        </p>
        <h2>会议概况</h2>
        <Table
          rowKey="id"
          className="approval-preview-table"
          columns={meetingInfoColumns}
          dataSource={infoData.meetingInfo || []}
          pagination={false}
          bordered
        />
        <h2>议案信息</h2>
        ...
        {(infoData.assessmentList || []).map((motion, index) => {
          ...
          return (
            <div className="approval-preview-topic" key={motion.title || index}>
              <h3>{motion.title}</h3>
              <div className="approval-preview-image-list">
                {approvalPreviewImages.map((img) => (
                  <img key={img.id} src={img.fileUrl} alt={img.fileName} />
                ))}
              </div>
              <Table
                rowKey="id"
                className="approval-preview-table"
                columns={assessmentColumns(rows)}
                dataSource={rows}
                pagination={false}
                bordered
                summary={() => (
              ...
              <div className="approval-preview-line">
                <b>董监事的意见：</b>
                <HtmlText value={motion.commentList} />
              </div>
              <div className="approval-preview-line">
                <b>提请决策事项：</b>
                <HtmlText value={motion.todoComment} />
              </div>
            </div>
          );
        })}
        {infoData.commentList?.length ? (
          <>
            ...
            <Table
              ...
              dataSource={infoData.commentList || []}
              pagination={false}
              bordered
            />
          </>
        ...
        {infoData.topicOpnList?.length ? (
          <>
            <h2>提请决策事项</h2>
            {infoData.desc ? (
              <div className="approval-preview-desc">
                <HtmlText value={infoData.desc} />
              </div>
            ) : null}
            <Table
              rowKey="id"
              ...
              dataSource={infoData.topicOpnList || []}
              pagination={false}
              bordered
            />
          </>
        ) : null}
        {infoData.auth?.map((item, index) => (
          <div key={index} className="approval-preview-desc">
            授权{item.auth}在{item.mtName}上代表本公司行使表决权
          </div>
        ))}
        {infoData.sanhuiVote ? (
          <div className="approval-preview-desc">
            <HtmlText value={infoData.sanhuiVote} />
          </div>
        ) : null}
      </div>
    </Modal>
...
export function MaterialPreparePanel({
  isEdit,
  setActiveKey,
  onSubmitSuccess,
  submitButtonText = "下一步",
  afterContent,
  useStructuredPreview = false,
}) {
  const [form] = Form.useForm();
  const materialData = initialMaterialPrepare;
  const [loading, setLoading] = useState(false);
  const [tableData, setTableData] = useState(() =>
    normalizeDecisionRows(materialData.opnFormMap?.["100"] || []),
  );
  const [tableData1, setTableData1] = useState(() =>
    normalizeDecisionRows(materialData.opnFormMap?.["200"] || []),
  );
  const [tableData2, setTableData2] = useState(() =>
    normalizeDecisionRows(materialData.opnFormMap?.["300"] || []),
  );
  const [proposalList, setProposalList] = useState(
    materialData.proposalList || [],
  );
  ...
  const initialFormValues = useMemo(
    () => ({
      ...
      itemFormList:
        materialData.voteDTO?.itemFormList?.map((item) => item.userId) || [],
    }),
    [materialData],
  );
  const userOptions = useMemo(
    () =>
      (votePersonSelectResponse.data || []).map((item) => ({
  ...
  const getStatus = (result, maxResult, excludeFlag) => {
    if (excludeFlag) return <span className="status-dot status-green" />;
    if (result === null || result === undefined) return <span>--</span>;
    if (result >= 90) return <span className="status-dot status-green" />;
    if (result >= 80) return <span className="status-dot status-blue" />;
    if (result >= 70) return <span className="status-dot status-yellow" />;
  ...
  const updateVoteAdvice = (record, value, setter) => {
    setter((current) =>
      current.map((item) =>
        item.id === record.id ? { ...item, voteAdvice: value } : item,
      ),
    );
  };
      ...
      current.map((item) =>
        item.id === record.id
          ? {
              ...item,
              avoidVoteFlag: value,
              voteAdvice: value === "1" ? "" : item.voteAdvice,
            }
          : item,
      ),
    );
  ...
  const updateMgmtComment = (record, value, setter) => {
    setter((current) =>
      current.map((item) =>
        item.id === record.id ? { ...item, mgmtComment: value } : item,
      ),
    );
  };
  const changeTotalValue = (score, proposalName) => {
    ...
    [setTableData, setTableData1, setTableData2].forEach((setter) => {
      setter((current) =>
        current.map((item) =>
          item.sanhuiTopicName === proposalName && item.avoidVoteFlag !== "1"
            ? { ...item, voteAdvice: nextVote }
            : item,
        ),
      );
    });
  ...
  const updateProposal = (proposalId, updater) => {
    setProposalList((current) =>
      current.map((item) =>
        item.id === proposalId ? updater({ ...item }) : item,
      ),
    );
  };
  const buildPayload = (values) => ({
    ...
    proposalList,
    reviewMapFileList: Object.fromEntries(
      Object.entries(pdfUrl)
        .filter(([, value]) => value)
        .map(([key, value]) => [
          key,
          [{ fileName: previewTitleMap[key], fileUrl: value }],
        ]),
    ),
    voteDTO: {
  ...
  const createColumns = (setter) => [
    ...
    {
      title: "议案名称",
      dataIndex: "sanhuiTopicName",
      width: 260,
      render: (value, record) => (
        <Space size={4} wrap>
          <span>{value}</span>
          {record.topicType === "1" ? <Tag color="gold">股权投委会</Tag> : null}
        </Space>
      ),
    },
    {
      ...
      render: (value, record) => (
        <Select
          ...
          disabled={!isEdit}
          style={{ width: "100%" }}
          onChange={(nextValue) => updateAvoidVote(record, nextValue, setter)}
          options={[
            { label: "否", value: "0" },
            { label: "是", value: "1" },
          ]}
    ...
    },
    {
      ...
      render: (value, record) => {
        ...
        return (
          <Select
            value={isAvoidVote ? undefined : value || undefined}
            disabled={!isEdit || isAvoidVote}
            style={{ width: "100%" }}
            onChange={(nextValue) =>
              updateVoteAdvice(record, nextValue, setter)
            }
            options={[
              { label: "同意", value: "1" },
              { label: "反对", value: "2" },
    ...
    },
    {
      ...
      render: (value, record) => (
        <Input.TextArea
          ...
          onChange={(event) =>
            updateMgmtComment(record, event.target.value, setter)
          }
        />
      ),
    },
  ];
  const renderDecisionTable = (title, dataSource, setter) =>
    dataSource.length ? (
      <>
        ...
        <Table
          rowKey="id"
          bordered
          className="table-tabs1"
          pagination={false}
          columns={createColumns(setter)}
  ...
  const renderScoreTable = (scoreData, scoreIndex) => {
    ...
    const columns = [
      ...
      },
      {
        ...
        render: (_value, record, index) => ({
          children: record.sanhuiTopicModelFactorVo?.factorLv3Name || "",
          props: {
            rowSpan: calculateRowSpan(tableList, "factorLv3Name", index),
          },
        }),
      },
      {
        ...
        width: 120,
        render: (_value, record) =>
          record.sanhuiTopicModelFactorVo?.factorLv2Name || "",
      },
      {
        title: "评价要素",
        width: 250,
        render: (_value, record) =>
          record.sanhuiTopicModelFactorVo?.assessElement || "",
      },
      {
        title: "权重(%)",
        width: 90,
      ...
      },
      {
        ...
        width: 250,
        render: (_value, record) =>
          record.sanhuiTopicModelFactorVo?.criterion || "",
      },
      {
        title: "执行情况",
        ...
        render: (value, record, index) => (
          <Input.TextArea
            ...
            onChange={(event) =>
              updateProposal(scoreData.id, (draft) => {
                draft.topicAssesList[index] = {
                  ...record,
                  execDetail: event.target.value,
                };
                return draft;
              })
            }
      ...
      },
      {
        ...
        width: 250,
        render: (_value, record) =>
          record.sanhuiTopicModelFactorVo?.assessRule || "",
      },
      {
        title: "评价结果(分)",
            ...
            <Radio.Group
              ...
              onChange={(event) =>
                updateProposal(scoreData.id, (draft) => {
                  draft.topicAssesList[index] = {
                    ...record,
                    assessResult: event.target.value,
                  };
                  return draft;
                })
              }
            ...
            <Space>
              {record.excludeFlag !== "1" ? (
                <InputNumber
                  disabled={!isEdit}
                  min={0}
                  max={Number(record.sanhuiTopicModelFactorVo?.maxScore) || 100}
                  value={value}
                  onChange={(nextValue) =>
                    updateProposal(scoreData.id, (draft) => {
                      draft.topicAssesList[index] = {
                        ...record,
                        assessResult: nextValue,
                      };
                      return draft;
                    })
                  }
      ...
      },
      {
        ...
        render: (_value, record) =>
          getStatus(
            record.assessResult,
            Number(record.sanhuiTopicModelFactorVo?.maxScore) || 100,
            record.excludeFlag === "1",
          ),
      },
    ];
    const hasDisqualified = tableList.some(
      (item) =>
        item.sanhuiTopicModelFactorVo?.factorType === "2" &&
        Number(item.assessResult) === 0,
    );
    const hasValidFactors = tableList.some(
      (item) => item.sanhuiTopicModelFactorVo?.factorType === "1",
    );
    const total =
      hasDisqualified || !hasValidFactors
        ...
        : Number(
            tableList
              .filter(
                (item) => item.sanhuiTopicModelFactorVo?.factorType !== "2",
              )
              .reduce(
                (sum, item) =>
                  sum +
                  (Number(item.sanhuiTopicModelFactorVo?.weight) *
                    Number(
                      item.excludeFlag === "1" ? 0 : item.assessResult || 0,
                    )) /
                    100,
                0,
              )
    ...
    return (
      <div className="scoreTable" key={scoreData.id || scoreIndex}>
        ...
        <div className="img-wraper">
          ...
          <Button
            ...
            onClick={() => setMaterialOpen(scoreData)}
          >
            补充汇报材料
          </Button>
        ...
        <Form.Item
          label="董监事意见"
          name={`comment-${scoreData.id}`}
          initialValue={scoreData.comment}
        >
          <Input.TextArea
            disabled={!isEdit}
            autoSize
        ...
        <Form.Item
          label="提请决策事项"
          name={`decisionRequestItems-${scoreData.id}`}
          initialValue={scoreData.decisionRequestItems}
        >
          <Input.TextArea
            disabled={!isEdit}
            autoSize
            onChange={(event) =>
              updateProposal(scoreData.id, (draft) => ({
                ...draft,
                decisionRequestItems: event.target.value,
              }))
            }
          />
        </Form.Item>
  ...
  return (
    <div className="tabs-container-sanhui material-prepare-container">
      ...
      <div className="projectBtn">
        {pdfCurrentList.includes("1000") ? (
          <Button
            loading={loading}
            type="primary"
            onClick={() => (isEdit ? onSave(0, "1000") : onPreview("1000"))}
          >
            向总监汇报预览
          </Button>
        ) : null}
        {pdfCurrentList.includes("2000") ? (
          <Button
            loading={loading}
            type="primary"
            onClick={() => (isEdit ? onSave(0, "2000") : onPreview("2000"))}
          >
            向分管领导汇报预览
          </Button>
        ) : null}
        {pdfCurrentList.includes("3000") ? (
          <Button
            loading={loading}
            type="primary"
            onClick={() => (isEdit ? onSave(0, "3000") : onPreview("3000"))}
          >
            向投委会汇报预览
          </Button>
        ) : null}
        ...
        {isEdit ? (
          <Button
            ...
            type="primary"
            onClick={() =>
              onSubmitSuccess ? onSave(1) : setSubmitMeetingModalOpen(true)
            }
          >
            {submitButtonText}
          </Button>
      ...
      <Modal
        title="是否提请部务会"
        open={submitMeetingModalOpen}
        onCancel={() => setSubmitMeetingModalOpen(false)}
        footer={
          <Space>
            <Button
              onClick={() => {
      ...
      <Modal
        title={previewTitleMap[currentPreviewType]}
        open={pdfModalOpen}
        ...
        footer={
          <>
            {pdfUrl[currentPreviewType] ? (
              <div className="pdf-success">
                {previewSuccessMap[currentPreviewType]}
              </div>
            ) : null}
            <Button onClick={() => setPdfModalOpen(false)}>取消</Button>
            {pdfCurrent > 0 ? (
              <Button onClick={() => setPdfCurrent(pdfCurrent - 1)}>
                上一步
              </Button>
            ) : null}
            <Button
      ...
      />
      <SupplementMaterials
        open={Boolean(materialOpen)}
        onClose={() => setMaterialOpen(null)}
      />
    </div>
  );
}
function MinistryMeetingPanel({ isEdit, setActiveKey }) {
  ...
  return (
    <div className="review-panel ministry-meeting-panel">
        ...
        <div className="ministry-meeting-form-card">
          <Form
            ...
            initialValues={{
              ...
              reason:
                "该事项涉及基金退出路径、交易对手沟通和风险应对安排，需提请部务会审议。",
      ...
      {isEdit ? (
        <div className="projectBtn">
          <Button onClick={() => setActiveKey("1")}>上一步</Button>
          <Button
            type="primary"
            onClick={() => message.success("部务会提请信息已保存")}
          >
            保存
          </Button>
...
const officeApprovalSteps = [
  {
    role: "申请人",
    name: "杨佰君",
    time: "2026-04-21 18:44:33",
    state: "start",
  },
  {
    role: "总监",
    name: "黄国平",
    time: "2026-04-21 18:59:18",
    state: "approved",
    remark: "同意",
  },
  {
  ...
  },
  {
    role: "分管领导",
    name: "李秀柱",
    time: "2026-04-22 09:15:44",
    state: "approved",
    remark: "同意",
    muted: true,
  },
];
const investmentApprovalSteps = [
  {
    role: "申请人",
    name: "杨佰君",
    time: "2026-04-21 18:44:33",
...
function ApprovalStepList({ steps }) {
  return (
    <div className="approval-step-list">
      {steps.map((step, index, list) => (
        <div
          className={`approval-step-item ${step.state === "approved" ? "is-approved" : "is-start"} ${step.muted ? "is-muted" : ""}`}
        ...
        >
          ...
          <div className="approval-step-card">
            <div className="approval-step-card-head">
              ...
              {step.state === "approved" ? (
                <span className="approval-step-status">审批通过</span>
              ) : null}
            </div>
            <div className="approval-step-time">{step.time}</div>
            {step.remark ? (
              <div className="approval-step-remark">
                审批意见：{step.remark}
              </div>
            ) : null}
          </div>
...
function ReviewApprovalSteps({ steps = officeApprovalSteps }) {
  return (
    <div className="approval-step-panel">
...
function JointReviewApprovalTabs() {
  const items = [
    {
      key: "office",
      label: "总办会审批流",
      children: <ApprovalStepList steps={officeApprovalSteps} />,
    },
    {
      key: "investment",
      label: "股权投委会审批流",
      children: <ApprovalStepList steps={investmentApprovalSteps} />,
    },
  ];
  return (
    <div className="approval-step-panel joint-review-approval-tabs">
      <Tabs defaultActiveKey="office" items={items} size="small" />
    </div>
  );
}
function TopicApprovalFlowPanel({
  projectId,
  isEdit,
  onClosed,
  setActiveKey,
  investOrg,
  pdfCurrentList = ["1000", "2000", "3000"],
}) {
  const [form] = Form.useForm();
  const investmentScopeFlag = Form.useWatch("investmentScopeFlag", form);
  const [loading, setLoading] = useState(false);
  ...
  const fileNameMap = {
    ...
    2000: "分管领导审批议题材料.pdf",
    3000: "总办会审批议题材料.pdf",
    4000: "股权投委会审批议题材料.pdf",
  };
  const reviewFileKeys = ["1000", "2000", "3000", "4000"].filter(
    (key) => key === "4000" || pdfCurrentList.includes(key),
  );
  const normalizeUploadList = (list = []) =>
    list.map((file) => ({
  ...
  const generateReviewFile = (key) => {
    ...
    message.success(
      `${fileTypeMap[key]}材料已${(fileList[key] || []).length ? "重新" : ""}生成`,
    );
  };
    ...
    form
      ...
      .then((values) => {
        const data = {
          ...
          planStartDate:
            values.officeMeetingTime?.[0]?.format("YYYY-MM-DD HH:mm:ss") ||
            null,
          planEndDate:
            values.officeMeetingTime?.[1]?.format("YYYY-MM-DD HH:mm:ss") ||
            null,
          investmentPlanStartDate:
            values.investmentMeetingTime?.[0]?.format("YYYY-MM-DD HH:mm:ss") ||
            null,
          investmentPlanEndDate:
            values.investmentMeetingTime?.[1]?.format("YYYY-MM-DD HH:mm:ss") ||
            null,
  ...
  useEffect(() => {
    ...
    form.setFieldsValue({
      ...data,
      officeTopic: data.officeTopic || data.topic,
      investmentTopic: data.investmentTopic || data.topic,
      investmentScopeFlag: data.investmentScopeFlag || "1",
      investmentAgendaItem:
        data.investmentAgendaItem || investmentAgendaDefault,
      officeMeetingTime: data.planStartDate
        ? [dayjs(data.planStartDate), dayjs(data.planEndDate)]
        : null,
      investmentMeetingTime: data.investmentPlanStartDate
        ? [
            dayjs(data.investmentPlanStartDate),
            dayjs(data.investmentPlanEndDate),
          ]
        : null,
      thImptLargeFlag: data.thImptLargeFlag || "0",
      oaMeetingAttendeeList:
        data.oaMeetingAttendeeList?.map((item) => item.userId) || [],
    });
    ...
    setFileList({
      ...
      4000: data.reviewMapFileList?.["4000"] || [],
    });
    setFileImgList(data.decisionRequestScreenshotFile || []);
  }, []);
  return (
    <div className="tabs2-container tabs-container-sanhui topic-approval-layout">
          ...
          <Spin spinning={loading}>
            <Form layout="vertical" form={form} disabled={!isShow}>
              <Card
                size="small"
                title="股权投委会"
                className="topic-approval-meeting-card"
              >
                <Form.Item
                  label="股权投委会议题名称"
                  name="investmentTopic"
                  rules={[
                    { required: true, message: "请输入股权投委会议题名称" },
                  ]}
                >
                  <Input />
                </Form.Item>
                <div className="item-flex-wrap">
                  <Form.Item
                    label="是否为投委会议事范围"
                    name="investmentScopeFlag"
                    initialValue="1"
                    rules={[
                      { required: true, message: "请选择是否为投委会议事范围" },
                    ]}
                  >
                    <Radio.Group>
                  ...
                  <Form.Item label="拟上会时间" name="investmentMeetingTime">
                    <DatePicker.RangePicker showTime />
                  </Form.Item>
                </div>
                {investmentScopeFlag === "1" ? (
                  <Form.Item
                    label="议事事项"
                    name="investmentAgendaItem"
                    initialValue={investmentAgendaDefault}
                  >
              ...
              >
                <Form.Item
                  label="总办会议题名称"
                  name="officeTopic"
                  rules={[{ required: true, message: "请输入总办会议题名称" }]}
                >
                  <Input />
                </Form.Item>
                <div className="item-flex-wrap">
                  ...
                  >
                    <Radio.Group
                      onChange={(event) => setLargeFlag(event.target.value)}
                    >
                      <Radio value="0">是</Radio>
                      <Radio value="1">否</Radio>
                    </Radio.Group>
                  </Form.Item>
                  <Form.Item label="拟上会时间" name="officeMeetingTime">
                    <DatePicker.RangePicker showTime />
                  </Form.Item>
                </div>
                {largeFlag === "0" ? (
                  <Form.Item label="三重一大事项" name="thImptLargeType">
                    <Input />
                  </Form.Item>
                ) : null}
              </Card>
              ...
              <div className="item-flex-wrap">
                <Form.Item label="提报日期" name="applDate">
                  <Input disabled />
                </Form.Item>
              ...
              <div className="item-flex-wrap">
                    ...
                    onChange={(value) => {
                      ...
                      form.setFieldsValue({
                        oaMeetingAttendeeList: value.map((item) => ({
                          userId: item,
                          userName:
                            userOptions.find((option) => option.value === item)
                              ?.label || item,
                        })),
                      });
                    }}
                ...
                <Form.Item label="汇报人" name="presUserName">
                  <Input disabled />
                </Form.Item>
              </div>
              <div className="item-flex-wrap">
                <Form.Item label="法务联审" name="fwlsName">
                  <Input disabled />
                </Form.Item>
                <Form.Item label="综合管理联审" name="jhgllsName">
                  <Input disabled />
                </Form.Item>
              </div>
              <div className="item-flex-wrap">
                <Form.Item label="财务联审" name="cwlsName">
                  <Input disabled />
                </Form.Item>
                <Form.Item label="投资联审" name="tzlsName">
                  <Input disabled />
                </Form.Item>
              </div>
              <div className="item-flex-wrap">
                <Form.Item label="投资部2总监" name="investDeptTwo">
                  <Input disabled={!isShow} value={investOrg} />
                </Form.Item>
                <Form.Item label="党群联审" name="dqlsName">
                  <Input disabled />
                </Form.Item>
              </div>
              <div className="item-flex-wrap">
                      ...
                      <div key={key}>
                        <div className="file-upload-item">
                          ...
                          <Button
                            ...
                            ghost
                            loading={loading}
                            onClick={() => generateReviewFile(key)}
                          >
                            {hasFile ? "重新生成" : "生成"}
                          </Button>
                        </div>
                        <Upload
                          disabled={!isShow}
                          fileList={normalizeUploadList(fileList[key])}
                          beforeUpload={(file) => {
              ...
              <div className="item-flex-wrap">
                <Form.Item
                  label="提请决策事项截图"
                  name="decisionRequestScreenshotFile"
                >
                  <Upload
                    disabled={!isShow}
                    fileList={normalizeUploadList(fileImgList)}
                    beforeUpload={(file) => {
                      addUploadFile(setFileImgList, file);
                      return false;
                    }}
                    onRemove={(file) => {
                      setFileImgList((current) =>
                        current.filter(
                          (item) =>
                            (item.uid ||
                              item.fileUrl ||
...
function JointOpinionTable({ isEdit }) {
  ...
  return (
    <Table
      ...
      dataSource={rows}
      pagination={false}
      className="table-tabs1 joint-opinion-table"
      bordered
    />
  );
}
  ...
  const rows = [
    {
      title: "风控合规审核意见",
      content:
        "经审核，本议题决策依据、审批路径及材料完整性基本符合公司治理要求；建议在提交正式审批前补充交易对手资信复核记录，并同步完善资金回收节点责任分工。",
    },
    {
      title: "风控合规风险提示应对建议",
      content:
        "请重点关注退出协议履约、资金到账进度及信息披露一致性风险；建议设置阶段性跟踪台账，明确异常情况升级汇报机制，并在会后形成风险闭环记录。",
    },
  ];
  return (
    <div className="risk-compliance-opinions">
      {rows.map((item) => (
        <div className="risk-compliance-card" key={item.title}>
...
function JointOpinionPanel({ isEdit, setActiveKey }) {
  return (
    <div className="joint-review-layout tabs2-container tabs-container-sanhui">
...
function AfterMeetingReplacementApplication({ isEdit }) {
      ...
      <div className="after-meeting-archive-card">
        <div className="after-meeting-file-action">
          ...
          <Button
            ...
            disabled={!isEdit}
            onClick={() =>
              regenerateArchiveFile(setter, label, fileName, fileUrl)
            }
          >
            重新生成
          </Button>
  ...
  const initialValues = {
    ...
    "planStartDate-planEndDate": [
      dayjs("2026-05-08 09:00", "YYYY-MM-DD HH:mm"),
      dayjs("2026-05-08 11:00", "YYYY-MM-DD HH:mm"),
    ],
    planMinute: 20,
  };
  return (
    <div className="after-meeting-application">
          ...
          <Form.Item label="相关材料" name="relatedFile">
            ...
            <div className="after-meeting-file-action">
              ...
              <Button
                ...
                disabled={!isEdit}
                onClick={() =>
                  regenerateArchiveFile(
                    setFileList,
                    "总办会决策文件",
                    "总办会决策文件.pdf",
                    "/mock-pdf/after-meeting-replacement.pdf",
                  )
                }
              >
                重新生成
            ...
            <Upload
              disabled={!isEdit}
              fileList={normalizeUploadList(fileList)}
              beforeUpload={(file) => {
...
function PostMeetingMaterialPanel({ isEdit, setActiveKey }) {
  return (
    <div className="joint-review-layout tabs2-container tabs-container-sanhui post-meeting-material-layout">
      <div className="joint-review-left">
        <div className="joint-review-workspace">
          <div className="joint-opinion-section">
            <div className="questions-title">
          ...
          <div className="joint-material-section">
            <MaterialPreparePanel
              ...
              afterContent={
                <AfterMeetingReplacementApplication isEdit={isEdit} />
              }
  ...
  const [files, setFiles] = useState([
    {
      ...
      name: "会后修订版议案.pdf",
      type: "会后材料",
      updated: "2026-04-30 16:20",
    },
    {
      id: "post-002",
      name: "决议事项执行清单.xlsx",
      type: "执行清单",
      updated: "2026-05-01 09:40",
    },
  ]);
  return (
...
function MeetingMinutesPanel({
  ...
  const [officeDecisionData, setOfficeDecisionData] = useState([]);
  const [investmentDecisionData, setInvestmentDecisionData] = useState([]);
  const [officeFileList, setOfficeFileList] = useState([]);
  const [investmentFileList, setInvestmentFileList] = useState([]);
  const [isShowTip] = useState(false);
  const normalizeDecisionList = (list = []) =>
    list.map((item) => ({
      ...item,
      directorPassFlag:
        item.directorPassFlag || item.dsPassFlag || item.gqPassFlag,
      supervisorPassFlag:
        item.supervisorPassFlag || item.jsPassFlag || item.gqPassFlag,
      shareholderPassFlag:
        item.shareholderPassFlag || item.gdPassFlag || item.gqPassFlag,
    }));
  const getDecisionFormValues = (list, prefix) =>
    list.reduce(
      (values, item, index) => ({
        ...values,
        [`${prefix}DirectorPassFlag${index}`]: item.directorPassFlag,
        [`${prefix}SupervisorPassFlag${index}`]: item.supervisorPassFlag,
        [`${prefix}ShareholderPassFlag${index}`]: item.shareholderPassFlag,
      }),
      {},
    );
  const getList = () => {
    ...
    setTimeout(() => {
      ...
      const officeDecisions = normalizeDecisionList(
        data.sanhuiCoReviewGoDecisionVos,
      );
      const investmentDecisions = normalizeDecisionList(
        data.investmentDecisionVos || data.sanhuiCoReviewGoDecisionVos,
      );
      setOfficeDecisionData(officeDecisions);
      setInvestmentDecisionData(investmentDecisions);
      setOfficeFileList(data.fileList || []);
      setInvestmentFileList(data.investmentFileList || []);
      form.setFieldsValue({
        ...getDecisionFormValues(officeDecisions, "office"),
        ...getDecisionFormValues(investmentDecisions, "investment"),
        zbhLaunchDate: data.zbhLaunchDate
          ? dayjs(data.zbhLaunchDate, "YYYY-MM-DD")
          : undefined,
        zbhIssueNo: data.zbhIssueNo,
        investmentLaunchDate: data.investmentLaunchDate
          ? dayjs(data.investmentLaunchDate, "YYYY-MM-DD")
          : undefined,
        investmentIssueNo: data.investmentIssueNo,
      });
  ...
  const onSave = (type) => {
    const params = {
      mgmtId: projectId,
      sanhuiCoReviewGoDecisionVos: officeDecisionData,
      investmentDecisionVos: investmentDecisionData,
    ...
    form
      .validateFields()
      .then((values) => {
        params.zbhLaunchDate = values.zbhLaunchDate
          ? values.zbhLaunchDate.format("YYYY-MM-DD")
          : undefined;
        params.zbhIssueNo = values.zbhIssueNo;
        params.investmentLaunchDate = values.investmentLaunchDate
          ? values.investmentLaunchDate.format("YYYY-MM-DD")
          : undefined;
        params.investmentIssueNo = values.investmentIssueNo;
        setTimeout(() => {
  ...
  const updateMeetingDecision = (setDecisionData, index, fieldName, value) => {
    setDecisionData((current) =>
  ...
  const renderMeetingSection = ({
    ...
    <div className="meeting-result-block">
      <div className="meeting-minutes-basic">
        <div className="questions-title">
          <span className="title-dot" />
          <span className="title-text">{infoTitle}</span>
        </div>
        <div className="meeting-basic-grid">
          ...
          <Form.Item label="期数" name={issueName}>
            <InputNumber
              min={1}
              precision={0}
              placeholder="请输入期数"
          ...
          <Form.Item label="会议纪要" className="formItem-upload">
            <Upload
      ...
      <div className="meeting-decision-section">
        <div className="questions-title">
          <span className="title-dot" />
          <span className="title-text">{decisionTitle}</span>
        </div>
        {isShowTip && fileList.length > 0 && (
          <div className="jingshi-title">
            <span className="jingshi-text">
              AI已经从会议纪要中提取了各议题的会议决策，请确认并补充遗漏或修改错误！
            </span>
          </div>
        )}
        <Spin spinning={tableLoading}>
          <div className="meeting-decision-card-list">
            {decisionData.map((record, index) => {
              const topic = record.eoSanhuiTopic || {};
              return (
                <div
                  className="meeting-decision-card"
                  key={
                    record.id ||
                    `${fieldPrefix}-${topic.categoryLv3Name}-${topic.toipcName}`
                  }
                >
                  <div className="meeting-decision-card-head">
                    ...
                    <div className="meeting-decision-topic">
                      <div className="meeting-decision-topic-name">
                        {topic.toipcName || "未命名议题"}
                      </div>
                    </div>
                  </div>
                  <div className="meeting-vote-grid">
                    {[
                      {
                        label: "董事会",
                        field: "directorPassFlag",
                        message: "请选择董事会表决建议",
                      },
                      {
                        label: "监事会",
                        field: "supervisorPassFlag",
                        message: "请选择监事会表决建议",
                      },
                      {
                        label: "股东会/投委会",
                        field: "shareholderPassFlag",
                        message: "请选择股东会/投委会表决建议",
                      },
                    ].map((vote) => (
                      <Form.Item
                        key={vote.field}
                        label={vote.label}
                        name={`${fieldPrefix}${vote.field[0].toUpperCase()}${vote.field.slice(1)}${index}`}
                        initialValue={record[vote.field]}
                        rules={[{ required: true, message: vote.message }]}
                      >
                        <Radio.Group
                          disabled={!isEdit}
                          options={meetingDecisionVoteOptions}
                          value={record[vote.field]}
                          onChange={(event) =>
  ...
  return (
    <div className="tabs4-container tabs-container-sanhui">
      <div className="approval-table">
        <Form layout="vertical" form={form}>
          {renderMeetingSection({
            infoTitle: "总办会会议信息",
            decisionTitle: "总办会会议决策",
            dateLabel: "总办会召开日",
            dateName: "zbhLaunchDate",
            issueName: "zbhIssueNo",
            fileList: officeFileList,
            setFileList: setOfficeFileList,
            decisionData: officeDecisionData,
            setDecisionData: setOfficeDecisionData,
            fieldPrefix: "office",
          })}
          {renderMeetingSection({
            infoTitle: "股权投委会会议信息",
            decisionTitle: "股权投委会会议决策",
            dateLabel: "股权投委会召开日",
            dateName: "investmentLaunchDate",
            issueName: "investmentIssueNo",
            fileList: investmentFileList,
            setFileList: setInvestmentFileList,
            decisionData: investmentDecisionData,
            setDecisionData: setInvestmentDecisionData,
            fieldPrefix: "investment",
          })}
        </Form>
      </div>
      <div className="projectBtn">
        ...
        {isEdit && (
          <Button type="primary" loading={loading} onClick={() => onSave("1")}>
            提交
          </Button>
        )}
      </div>
...
export default function CompanyReview({
  projectId,
  isEdit,
  projectData = companyReviewDetailResponse.data,
  initialActiveKey = "1",
  onClosed,
  onActiveKeyChange,
}) {
          ...
          <span className="review-tab-label-with-help">
            ...
            <Tooltip title="1.现在要显示所有的议题">
              <QuestionCircleOutlined
                className="review-tab-help-icon"
