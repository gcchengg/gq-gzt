  ...
  const steps = [
    {
      role: "申请人",
      name: "郑华峰",
      time: "2026-04-27 10:30:21",
      type: "start",
    },
    {
      role: "科室经理",
      name: "郑华峰",
      time: "2026-04-28 19:18:09",
      type: "pass",
      opinion: "审批通过",
    },
  ];
  ...
  return (
    <aside className="topic-report-approval">
      ...
      <div className="topic-report-timeline">
        {steps.map((step) => (
          <div
            className={`topic-report-time-item ${step.type}`}
          ...
          >
            <div className="topic-report-time-main">
              ...
              {step.type === "pass" ? (
                <span className="topic-report-pass">审批通过</span>
              ) : null}
            </div>
            <div className="topic-report-time-date">{step.time}</div>
            {step.opinion ? (
              <div className="topic-report-time-opinion">
                {step.type === "pass" ? "审批意见：" : ""}
                {step.opinion}
              </div>
            ) : null}
...
const DocumentFiling = (props, ref) => {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    Promise.all([
      getMatName({ sanhuiId: props.sanhuiMgmtId }),
      groupMessageDlvyOAGet({ sanhuiId: props.sanhuiMgmtId }),
    ])
      .then(([matNameResult, result]) => {
        ...
        const uploadList = (data.uploadFileList || []).map((item) => ({
          ...
          status: "done",
          response: {
            data: { originalFilename: item.fileName, url: item.fileUrl },
          },
        }));
        ...
        setFormObj({
          submission: Boolean(
            data.matName || data.matDesc || uploadList.length,
          ),
        });
  ...
  const formatParams = async () => {
    ...
    return {
      ...
      uploadFileList: fileList.map((item) => ({
        fileName: item.name,
        fileUrl:
          item.url || item.response?.data?.url || "/mock-files/uploaded.pdf",
  ...
  const beforeUpload = (file) => {
    ...
    const item = {
      ...
      response: {
        data: { originalFilename: file.name, url: URL.createObjectURL(file) },
      },
    };
      ...
      <Spin spinning={loading}>
        ...
        >
          ...
          <span className="topic-report-switch-text">
            {formObj.submission
              ? "有（需要向集团总经理助理及以上董监事汇报）"
              : "无（本参股企业没有集团总经理助理及以上董监事）"}
          </span>
        </Form.Item>
        <div className="topic-report-form-grid">
          <div className="topic-report-scroll">
            ...
            >
              <Radio.Group disabled={disabled}>
                <Space direction="vertical">
                  ...
                  <Radio value="1">
                    报告（只是向集团公司领导汇报某一事项，仅需领导阅知，不需批示的材料）
                  </Radio>
                </Space>
              </Radio.Group>
            </Form.Item>
            ...
            <p className="topic-report-help">
              申请集团非公文报送Word版材料需加盖股权公司行政公章（党委发文需加盖公司党委章）
            </p>
            <Row gutter={16}>
              <Col span={12}>
                <Form.Item
              ...
              <Col span={12}>
                <Form.Item name="recvOrg" rules={[{ required: true }]}>
                  <Select
                    disabled={disabled}
                    options={[
                      { value: "0", label: "报送集团公司领导" },
                      { value: "1", label: "报送集团公司职能部" },
                    ]}
                  />
                </Form.Item>
              </Col>
            ...
            <div className="topic-report-required">
              核心内容（材料具体描述）
            </div>
            <p className="topic-report-help">
              核心内容样例：1. 所呈材料的中心内容。2. 现呈报XX领导阅示。
            </p>
            <Form.Item
              name="matDesc"
