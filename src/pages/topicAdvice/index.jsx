...
const topicFeedbackRecords = {
  "topic-001": [
    ...
    },
    {
      ...
      content:
        "已收到，管户团队正在补充资信核查表和收益测算口径，预计今日 16:00 前完成材料更新。",
    },
  ],
  "topic-002": [
    {
    ...
    },
    {
      ...
      content:
        "已核对章程及投资协议，不触发其他股东优先购买权。书面意见已随补充材料上传。",
    },
  ],
};
...
const initialTopicReplies = {
  "topic-001":
    "<p>建议围绕基金退出节奏、交易对手资信及年度收益影响进一步补充说明。</p>",
...
function ReadonlySection({ title, children }) {
  return (
    <section className="topic-advice-section">
...
function RichReplyEditor({ value, onChange }) {
  ...
  return (
    <div className="topic-rich-editor">
      ...
      <div
        ref={editorRef}
        className="topic-rich-body"
        contentEditable
        data-placeholder="请输入反馈建议意见，可使用上方工具进行简单排版"
...
export default function TopicAdvicePage() {
        ...
        render: (value) =>
          value?.toLowerCase().endsWith(".pdf") ? (
            <Button
              ...
              onClick={() =>
                setPdfEditor({ fileName: value, mode: "annotation" })
              }
            >
              {value}
            </Button>
  ...
  const handleSave = () => {
    ...
    setSavedAtByTopic(
      Object.fromEntries(topics.map((topic) => [topic.id, savedAt])),
    );
  ...
  return (
    <div className="topic-advice-page">
      ...
      <main className="topic-advice-content">
        <section className="topic-advice-overview">
          {summaryStats.map((item) => (
            <div className="topic-advice-stat" key={item.key}>
        ...
        <ReadonlySection title="会议管理">
          <div className="topic-meeting-grid">
            {meetings.map((meeting) => (
              <article className="topic-meeting-card" key={meeting.key}>
                ...
                <Descriptions
                  ...
                  items={[
                    ...
                    },
                    {
                      ...
                      label: "召开方式",
                      children:
                        meeting.launchType === 1 ? "现场会议" : "通讯表决",
                    },
                    {
                      key: "launchTime",
                      label: meeting.launchType === 2 ? "表决日期" : "会议时间",
                      children: meeting.launchTime.format(
                        meeting.launchType === 2
                          ? "YYYY-MM-DD"
                          : "YYYY-MM-DD HH:mm",
                      ),
                    },
        ...
        <section className="topic-advice-section topic-feedback-section">
          ...
          <div className="topic-feedback-topic-list">
            {topics.map((topic, index) => {
              ...
              return (
                <article className="topic-feedback-topic" key={topic.id}>
                  ...
                  <div
                    className="topic-feedback-topic-body"
                    hidden={isCollapsed}
                  >
                    <div className="topic-reference-files">
                      <div className="topic-reference-files-head">
                        ...
                        <Upload
                          ...
                          beforeUpload={(file) => addTopicFile(topic.id, file)}
                        >
                    ...
                    <div className="topic-feedback-list">
                      {(topicFeedbackRecords[topic.id] || []).map((item) => (
                        <div
                          className={`topic-feedback-row ${item.role === "manager" ? "is-manager" : "is-leader"}`}
                          key={item.id}
                        >
                          ...
                          <div className="topic-feedback-bubble">
                            {item.content}
                          </div>
                        </div>
                      ))}
                    ...
                    <div className="topic-feedback-editor">
                      ...
                      <RichReplyEditor
                        ...
                        onChange={(value) => updateTopicReply(topic.id, value)}
                      />
                    </div>
                  </div>
                </article>
