...
export default function GztDemo() {
  ...
  useEffect(() => {
    ...
    const evalHomeTabs = Array.from(
      document.querySelectorAll("[data-eval-home-tab]"),
    );
    const evalHomePanes = Array.from(
      document.querySelectorAll("[data-eval-home-pane]"),
    );
    const auditTabs = Array.from(document.querySelectorAll("[data-audit-tab]"));
    const auditPanes = Array.from(
      document.querySelectorAll("[data-audit-pane]"),
    );
    const workflowPanes = Array.from(
      document.querySelectorAll("[data-workflow-pane]"),
    );
    const auditMinutesOverlay = document.getElementById(
      "audit-minutes-overlay",
    );
    const auditMinutesMask = auditMinutesOverlay?.querySelector(
      ".audit-minutes-mask",
    );
    const auditMinutesClose = document.getElementById("audit-minutes-close");
    const auditMinutesOpeners = Array.from(
      document.querySelectorAll("[data-open-audit-minutes]"),
    );
    const evalDetailOverlay = document.getElementById("eval-detail-overlay");
    const evalDetailMask =
      evalDetailOverlay?.querySelector(".eval-detail-mask");
    const evalDetailClose = document.getElementById("eval-detail-close");
    const evalDetailTabs = Array.from(
      document.querySelectorAll("[data-eval-detail-tab]"),
    );
    const evalDetailPanes = Array.from(
      document.querySelectorAll("[data-eval-detail-pane]"),
    );
    const evalOpeners = Array.from(
      document.querySelectorAll("[data-open-eval-detail]"),
    );
    const evalPrevNext = Array.from(
      document.querySelectorAll("[data-eval-next], [data-eval-prev]"),
    );
    const evalModalOpeners = Array.from(
      document.querySelectorAll("[data-open-eval-modal]"),
    );
    const evalModalClosers = Array.from(
      document.querySelectorAll("[data-close-eval-modal]"),
    );
    ...
    const evalSupplementOverlay = document.getElementById(
      "eval-supplement-overlay",
    );
    ...
    const closeEvalSupplement = document.getElementById(
      "close-eval-supplement",
    );
    ...
    const pdfLinkConfirmCancel = document.getElementById(
      "pdf-link-confirm-cancel",
    );
    ...
    const pdfFileOpeners = Array.from(
      document.querySelectorAll("[data-open-pdf-editor]"),
    );
    const evalAnnotationOpeners = Array.from(
      document.querySelectorAll("[data-open-pdf-annotation]"),
    );
    const pdfModeButtons = Array.from(
      document.querySelectorAll("[data-pdf-mode]"),
    );
    const pdfPageChips = Array.from(
      document.querySelectorAll("[data-pdf-page-chip]"),
    );
    ...
    const pdfCanvasList = Array.from(
      document.querySelectorAll("[data-pdf-canvas]"),
    );
    const pdfTaskCards = () =>
      Array.from(document.querySelectorAll("[data-annotation-card]"));
    const pdfAnnotationNodes = () =>
      Array.from(
        document.querySelectorAll("[data-annotation-id], [data-selectable-id]"),
      );
    ...
    const pdfBottomAttachPanel = document.getElementById(
      "pdf-bottom-attach-panel",
    );
    const pdfBottomFileOpeners = () =>
      Array.from(
        document.querySelectorAll(
          ".pdf-bottom-attach-file[data-open-pdf-editor]",
        ),
      );
    const pdfBottomCompareButtons = () =>
      Array.from(document.querySelectorAll(".pdf-bottom-row-compare"));
    ...
    const pdfCompareInlineBack = document.getElementById(
      "pdf-compare-inline-back",
    );
    const pdfCompareOverlay = document.getElementById("pdf-compare-overlay");
    const pdfCompareMask =
      pdfCompareOverlay?.querySelector(".pdf-compare-mask");
    ...
    const evalAttachToolbarBtn = document.getElementById(
      "eval-attach-toolbar-btn",
    );
    ...
    const pdfNoteNeedReplyInputs = Array.from(
      document.querySelectorAll('input[name="pdf-note-need-reply"]'),
    );
    const progressOrder = [
      "submit",
      "evaluate",
    ...
    function activateRole(role) {
      const nextRole = ["manager", "finance", "legal"].includes(role)
        ? role
        : "manager";
      ...
      if (nextRole === "finance") {
        ...
        roleButtons.forEach((button) =>
          button.classList.toggle("active", button.dataset.role === "legal"),
        );
    ...
    function activateTab(name) {
      ...
      document.body.classList.remove("role-legal");
      roleButtons.forEach((button) =>
        button.classList.toggle("active", button.dataset.role === "manager"),
      );
      tabs.forEach((tab) =>
      ...
      footers.forEach((footer) =>
        footer.classList.toggle("active", footer.dataset.footer === name),
      );
      ...
      progressSteps.forEach((step, index) => {
        ...
        if (circle) {
          circle.textContent = isDone
            ? "✓"
            : progressNumbers[step.dataset.step] || "";
        }
      });
      if (stepName === "submit") {
        activateTab(
          document.querySelector(".sub-tab.active")?.dataset.tab || "ai",
        );
    ...
    function activateWorkflowPane(name) {
      workflowPanes.forEach((pane) =>
        pane.classList.toggle("active", pane.dataset.workflowPane === name),
      );
    }
    ...
    function activateEvalHomeTab(name) {
      evalHomeTabs.forEach((tab) =>
        tab.classList.toggle("active", tab.dataset.evalHomeTab === name),
      );
      evalHomePanes.forEach((pane) =>
        pane.classList.toggle("active", pane.dataset.evalHomePane === name),
      );
    }

    function activateAuditTab(name) {
      auditTabs.forEach((tab) =>
        tab.classList.toggle("active", tab.dataset.auditTab === name),
      );
      auditPanes.forEach((pane) =>
        pane.classList.toggle("active", pane.dataset.auditPane === name),
      );
    }

    function activateEvalDetailTab(name) {
      evalDetailTabs.forEach((tab) =>
        tab.classList.toggle("active", tab.dataset.evalDetailTab === name),
      );
      evalDetailPanes.forEach((pane) =>
        pane.classList.toggle("active", pane.dataset.evalDetailPane === name),
      );
    }
    ...
    function openEvalModal(name) {
      if (name === "attach" || name === "annotation") {
        evalAttachMode = name;
        if (evalAttachTitle)
          evalAttachTitle.textContent =
            name === "annotation" ? "关联批注" : "附件确认";
        if (evalAttachNote) {
          evalAttachNote.textContent =
            name === "annotation"
              ? "请选择需要关联批注的 PDF 附件。此处不支持重新上传附件，也不可更改勾选附件。"
              : "请勾选当前议题需要关联的附件，PDF 文件可点击进入批注编辑。";
        }
        if (evalAttachToolbarBtn) {
          evalAttachToolbarBtn.textContent =
            name === "annotation" ? "已关联附件" : "关联附件";
          evalAttachToolbarBtn.style.display =
            name === "annotation" ? "none" : "inline-flex";
        }
    ...
    function syncAttachTrigger() {
      ...
      evalAttachTrigger.textContent = evalAttachLinked
        ? "关联附件"
        : "未关联附件";
    }
    ...
    function setPdfMode(mode) {
      ...
      pdfCanvasList.forEach((canvas) => {
        ...
        canvas.classList.toggle(
          "idle",
          !pdfToolbarVisible || !["area", "new"].includes(mode),
        );
      });
    ...
    function setPdfToolbarVisible(visible) {
      ...
      pdfCanvasList.forEach((canvas) => {
        canvas.classList.toggle(
          "idle",
          !visible || !["area", "new"].includes(pdfMode),
        );
      });
    }
    ...
    function syncPdfBottomDock() {
      if (!pdfBottomDock) return;
      pdfBottomDock.classList.toggle(
        "annotation-mode",
        currentPdfContext === "annotation",
      );
    }
    ...
    function syncPdfContextUI() {
      ...
      if (pdfToolbarLabel) {
        pdfToolbarLabel.textContent = isAnnotation
          ? "关联类别："
          : "标注模式：";
      }
      if (pdfSideTitle) {
    ...
    function syncPdfProposalPanel() {
      ...
      pdfBottomAttachPanel.classList.toggle(
        "visible",
        !!pdfProposalSelect.value,
      );
    }
    ...
    function openPdfEditor(scope = "full", context = "attach", options = {}) {
      ...
      if (currentPdfContext !== "annotation" && pdfTaskAdd) {
        pdfTaskAdd.style.display =
          scope === "annotated" ? "none" : "inline-flex";
      }
    }

    function openPdfEditorFromFileTrigger(trigger) {
      const isReviewPageFile = trigger?.closest(
        ".finance-review-page, .legal-review-page",
      );
      if (isReviewPageFile) {
        openPdfEditor("full", "attach", {
          returnToReviewPage: true,
          reviewSource: true,
        });
        return;
      }
      openPdfEditor(
        evalAttachMode === "annotation" ? "annotated" : "full",
        evalAttachMode === "annotation" ? "annotation" : "attach",
      );
    }
    ...
    function clearPdfDraft() {
      ...
      document
        .querySelectorAll(".pdf-paragraph.text-target")
        .forEach((node) => node.classList.remove("text-target"));
      pdfDraft = null;
      if (pdfNotePop) pdfNotePop.style.display = "none";
      pdfNotePop?.classList.remove("centered");
      if (pdfNoteInput) pdfNoteInput.value = "";
      const defaultReplyType = pdfNoteNeedReplyInputs.find(
    ...
    function syncPdfTaskActionLabels() {
      document
        .querySelectorAll(".pdf-task-actions span:not(.danger)")
        .forEach((node) => {
    ...
    function positionPdfNotePop(pageNode, anchor) {
      ...
      const popHeight = 240;
      const left = Math.min(
        Math.max(32, anchor.left),
        pageNode.clientWidth - popWidth - 32,
      );
      const top = Math.min(
        Math.max(32, anchor.top),
        pageNode.clientHeight - popHeight - 32,
      );
    ...
    function syncPdfPageChip(page) {
      pdfPageChips.forEach((chip) =>
        chip.classList.toggle(
          "active",
          chip.dataset.pdfPageChip === String(page),
        ),
      );
      if (pdfPageInput) pdfPageInput.value = String(page);
      if (pdfPageTotal)
        pdfPageTotal.textContent = `/ ${visiblePdfPages().length}`;
    }

    function syncPdfPageLinkToggle() {
      if (!pdfPageLinkToggle) return;
      pdfPageLinkToggle.checked =
        currentPdfContext === "annotation" &&
        pdfLinkedPages.has(currentPdfPage);
    }
    ...
    function setActivePdfPage(page) {
      ...
      if (pdfPageNext)
        pdfPageNext.disabled = currentPdfPage === String(pages.length);
      if (pdfPageMinus) pdfPageMinus.disabled = currentPdfPage === "1";
      if (pdfPagePlus)
        pdfPagePlus.disabled = currentPdfPage === String(pages.length);
    }

    function activatePdfAnnotation(id) {
      if (!id) return;
      document
        .querySelectorAll(".pdf-task-card")
        .forEach((card) =>
          card.classList.toggle("active", card.dataset.annotationCard === id),
        );
      document
        .querySelectorAll(".pdf-annotation-box")
        .forEach((node) =>
          node.classList.toggle("active", node.dataset.annotationId === id),
        );
      document
        .querySelectorAll(".pdf-selectable")
        .forEach((node) =>
          node.classList.toggle("active", node.dataset.selectableId === id),
        );
      const target = document.querySelector(
        `[data-annotation-id="${id}"], [data-selectable-id="${id}"]`,
      );
      const card = document.querySelector(`[data-annotation-card="${id}"]`);
      const page =
        target?.dataset.page ||
        target?.closest("[data-pdf-page]")?.dataset.pdfPage;
      if (card) card.scrollIntoView({ behavior: "smooth", block: "nearest" });
      if (target) {
        if (page) {
          const pages = visiblePdfPages();
          const index = pages.findIndex(
            (node) => node.dataset.pdfPage === String(page),
          );
    ...
    function bindDiscussionCard(card) {
      ...
      function openComposer(mode) {
        ...
        textarea.placeholder =
          mode === "reply" ? "请输入回复内容" : "请输入建议内容";
        if (stateText)
          stateText.textContent =
            mode === "reply" ? "回复最近一轮建议" : "新增一轮建议";
        textarea.focus();
      }
    ...
    function visiblePdfPages() {
      if (currentPdfScope === "unannotated") {
        return pdfPages.filter(
          (page) => !(page.dataset.pdfScope || "full").includes("annotated"),
        );
      }
      return pdfPages.filter((page) =>
        (page.dataset.pdfScope || "full").includes(currentPdfScope),
      );
    }
    ...
    function savePdfDraft() {
      ...
      const needReply = pdfNoteNeedReplyInputs.some(
    ...
    progressSteps.forEach((step) => {
      step.addEventListener("click", () =>
        activateProgressStep(step.dataset.step),
      );
    });

    evalHomeTabs.forEach((tab) => {
      tab.addEventListener("click", () =>
        activateEvalHomeTab(tab.dataset.evalHomeTab),
      );
    });

    auditTabs.forEach((tab) => {
      tab.addEventListener("click", () =>
        activateAuditTab(tab.dataset.auditTab),
      );
    });
    ...
    evalDetailTabs.forEach((tab) => {
      tab.addEventListener("click", () =>
        activateEvalDetailTab(tab.dataset.evalDetailTab),
      );
    });
    ...
    evalPrevNext.forEach((button) => {
      button.addEventListener("click", () => {
        if (button.dataset.evalNext)
          activateEvalDetailTab(button.dataset.evalNext);
        if (button.dataset.evalPrev)
          activateEvalDetailTab(button.dataset.evalPrev);
      });
    });

    evalModalOpeners.forEach((button) => {
      button.addEventListener("click", () =>
        openEvalModal(button.dataset.openEvalModal),
      );
    });
    ...
    function bindBottomDockFileActions() {
      pdfBottomFileOpeners().forEach((button) => {
        ...
        button.addEventListener("click", () =>
          openPdfEditor(
            currentPdfContext === "annotation" ? "annotated" : "full",
            currentPdfContext,
          ),
        );
      });
    ...
    pdfFileOpeners.forEach((button) => {
      button.addEventListener("click", () =>
        openPdfEditorFromFileTrigger(button),
      );
    });
    ...
    evalAnnotationOpeners.forEach((button) => {
      button.addEventListener("click", () =>
        openPdfEditor("annotated", "annotation"),
      );
    });
    ...
    pdfPageInput?.addEventListener("change", () => {
      const page = Math.min(
        visiblePdfPages().length,
        Math.max(1, Number(pdfPageInput.value.replace(/\D/g, "")) || 1),
      );
    ...
    document.querySelectorAll(".pdf-selectable").forEach((node) => {
      if (node.dataset.selectableId && !node.dataset.noHighlight)
      ...
      node.addEventListener("click", (event) => {
        ...
        if (node.dataset.selectableId)
          activatePdfAnnotation(node.dataset.selectableId);
      });
    });
    ...
    pdfTaskCards().forEach((card) => {
      card.addEventListener("click", () =>
        activatePdfAnnotation(card.dataset.annotationCard),
      );
      bindPdfTaskCard(card);
    });
    ...
    document
      .querySelectorAll(
        "[data-compare-prev], [data-compare-next], [data-compare-minus], [data-compare-plus]",
      )
      .forEach((button) => {
        button.addEventListener("click", () => {
          const id =
            button.dataset.comparePrev ||
            button.dataset.compareNext ||
            button.dataset.compareMinus ||
            button.dataset.comparePlus;
          const delta =
            button.dataset.comparePrev || button.dataset.compareMinus ? -1 : 1;
          comparePagerState[id] = Math.min(
            10,
            Math.max(1, (comparePagerState[id] || 1) + delta),
          );
          syncComparePager(id);
        });
      });
    ...
    document.querySelectorAll("[data-compare-input]").forEach((input) => {
      input.addEventListener("change", () => {
        ...
        comparePagerState[id] = Math.min(
          10,
          Math.max(1, Number(input.value.replace(/\\D/g, "")) || 1),
        );
        syncComparePager(id);
      });
    });
    ...
    document
      .querySelectorAll(".pdf-compare-drawer .pdf-task-actions span")
      .forEach((node) => {
      ...
      <div className="viewport" id="viewport">
        <div className="workspace">
          <div className="topbar">
            ...
            <div className="role-switcher" aria-label="角色切换">
              ...
              <button className="role-btn" data-role="finance" type="button">
                财务
              </button>
              <button className="role-btn" data-role="legal" type="button">
                法务
              </button>
            </div>
        ...
        <main className="drawer" id="drawer">
          ...
          <nav className="sub-tabs" aria-label="议题提报页签">
            <button className="sub-tab ai active" data-tab="ai">
              <span className="ai-logo">Ai</span>智能识别
            </button>
            <button className="sub-tab" data-tab="topics">
              <span className="stack-icon"></span>议题管理
            </button>
            <button className="sub-tab" data-tab="meetings">
              <span className="people-icon">
                <span></span>
              </span>
              会议管理
            </button>
            <button className="sub-tab" data-tab="materials">
              <span className="doc-icon"></span>议题资料传达
            </button>
          </nav>
            ...
            <div className="ai-summary">
              ...
              <div className="upload-box">
                <div className="upload-icon">
                  <span className="upload-tray"></span>
                </div>
                <div className="upload-copy">点击或将文件拖拽到这里上传</div>
                <div className="upload-note">
                  <span className="warn-icon"></span>
                  请上传提报文档，AI将会提取信息为您自动创建议题以及会议数据！
                </div>
              </div>
            </div>
                ...
                <tbody>
                  <tr>
                    ...
                    <td className="blue-link">
                      1.招标文件-备注版-明阳智能AI在多业务
                      <br />
                      域应用试点项目-20250606.docx
                    </td>
                    <td>
                      <div className="fake-select">
                        议题相关<span className="chev"></span>
                      </div>
                    </td>
                    <td>解析完成</td>
                    <td>
                    ...
                    <td>
                      <div className="row-actions">
                        <span className="row-action">上移</span>
                        <span className="row-action">下移</span>
                        <span className="row-action danger">删除</span>
                      </div>
                    </td>
                  </tr>
                  <tr>
          ...
          <section className="panel" data-panel="topics">
            <div className="filter-row">
              <div className="filter-block">
                <label>议题分类（大）</label>
                <div className="fake-select placeholder">
                  请选择<span className="chev"></span>
                </div>
              </div>
              <div className="filter-block">
                <label>议题分类（中）</label>
            ...
            <button className="add-btn" id="open-topic-add" type="button">
              新增
            </button>
          ...
          <section
            className="topic-add-overlay"
            id="topic-add-overlay"
            aria-hidden="true"
          >
                ...
                <div className="topic-add-form">
                  <div className="topic-add-row topic-row-first">
                    ...
                    <div className="topic-field">
                      ...
                      <div className="topic-inline-box">
                        <div className="topic-radio-row">
                          <span className="topic-radio active">
                            <span className="topic-radio-dot"></span>有
                          </span>
                          <span className="topic-radio">
                            <span className="topic-radio-dot"></span>无
                          </span>
                        </div>
                      </div>
                    </div>
                    <div className="topic-field">
                      ...
                      <div className="topic-inline-box">
                        <div className="topic-radio-row">
                          <span className="topic-radio">
                            <span className="topic-radio-dot"></span>是
                          </span>
                          <span className="topic-radio active">
                            <span className="topic-radio-dot"></span>否
                          </span>
                        </div>
                      </div>
                    </div>
                    <div className="topic-field">
                      ...
                      <div className="topic-search placeholder">
                        <span>请选择关联的计划议题</span>
                  ...
                  <div className="topic-add-row topic-row-second">
                    <div className="topic-field">
                      ...
                      <div className="topic-select placeholder">
                        请选择<span className="chev"></span>
                      </div>
                    </div>
                    <div className="topic-field">
                    ...
                    <div className="topic-field">
                      ...
                      <div className="topic-inline-box toggle-box">
                        <div className="topic-segmented">
                          <div className="topic-segment active">
                            <span className="topic-segment-switch"></span>
                            <span>董事会</span>
                          </div>
                          <div className="topic-segment">
                            <span className="topic-segment-switch"></span>
                            <span>监事会</span>
                          </div>
                          <div className="topic-segment active">
                  ...
                  <div className="topic-attach-block">
                    <div className="topic-attach-head">
                      ...
                      <div className="notice topic-attach-note">
                        <span className="warn-icon"></span>
                  ...
                  <div className="topic-add-foot">
                    <button className="foot-btn primary wide" type="button">
                      补充汇报材料
                    </button>
                ...
                <div className="meeting-field method required">
                  ...
                  <div className="radio-row">
                    <div className="radio-item">
                      <span className="radio checked"></span>现场会议
                    </div>
                    <div className="radio-item">
                      <span className="radio"></span>通讯表决
                    </div>
                  </div>
                </div>
                ...
                <div className="joint-card">
                  <div className="joint-row">
                    ...
                    <div className="joint-input">
                      郑华峰<span className="search-icon"></span>
                    </div>
                  </div>
                  <div className="joint-row">
                    <div>
          ...
          <section className="evaluate-page" id="evaluate-page">
            <div className="eval-action-bar">
              <button className="eval-top-btn primary" type="button">
                表决授权
              </button>
              <button className="eval-top-btn primary" type="button">
                用印申请
            ...
            <div className="eval-section-tabs">
              <button
                className="eval-section-tab active"
                data-eval-home-tab="list"
                type="button"
              >
                <span className="eval-tab-icon check"></span>议题评估
              </button>
              <button
                className="eval-section-tab"
                data-eval-home-tab="qa"
                type="button"
              >
                <span className="eval-tab-icon qa"></span>
                议题初审问答(0件未处理)
              </button>
            </div>

            <div className="eval-home-pane active" data-eval-home-pane="list">
              <button className="eval-sort-btn" type="button">
                自动分级排序
              </button>
              <table className="data-table eval-home-table">
                ...
                <tbody>
                  <tr>
                    ...
                    <td className="category-small">
                      1.3.1
                      按国家部委等上级机构监管要求定期报告事项（含反洗钱、反欺诈、重大风险评估、绩效追索扣回、离任审计、内部控制等）
                    </td>
                    ...
                    <td className="meeting-flag">-</td>
                    <td className="center">
                      <span className="eval-status-pill">评估中</span>
                    </td>
                    <td className="op-cell">
                      <span className="eval-op-stack">
                        <span className="eval-op" data-open-eval-detail="1">
                          评估
                        </span>
              ...
              <button className="eval-finish-btn" type="button">
                评估完成
              </button>
            </div>
          ...
          <section className="audit-page" id="audit-page">
            <div className="audit-action-bar">
              <button
                className="audit-top-btn"
                data-open-audit-minutes
                type="button"
              >
                一汽股权会议纪要
              </button>
              <button className="audit-top-btn" type="button">
                投票结果
              </button>
              <button className="audit-top-btn" type="button">
                表决授权
            ...
            <div className="audit-body">
              <section className="audit-pane active" data-audit-pane="qa">
                <main className="audit-main">
                  ...
                  <div className="audit-toolbar">
                    ...
                    <button className="audit-primary-btn" type="button">
                      新增
                    </button>
                  </div>
                  ...
                  <div className="audit-empty">
                    <div className="audit-empty-box"></div>
                    <div>暂无数据</div>
                  </div>
                </main>
                <aside className="audit-side">
                  ...
                  <div className="audit-form-grid single">
                    <div className="audit-field">
                      <label>
                        <span className="required">*</span> 议题
                      </label>
                      <div className="audit-input muted"></div>
                    </div>
                    <div className="audit-field">
                      <label>提问部门</label>
                      <div className="audit-input muted"></div>
                    ...
                    <div className="audit-field">
                      <label>提问内容</label>
                      <div className="audit-textarea muted"></div>
                    </div>
                    <div className="audit-field">
                      <label>答复内容</label>
                      <div className="audit-textarea muted"></div>
              ...
              <section
                className="audit-pane no-side"
                data-audit-pane="materials"
              >
                <main className="audit-main">
                    ...
                    <table className="audit-table">
                      ...
                      <tbody>
                        <tr>
                          ...
                          <td>测试议题1</td>
                          <td>
                            <div className="audit-select audit-avoid-select">
                              是⌄
                            </div>
                          </td>
                          <td>
                            <div className="audit-select muted"></div>
                          </td>
                          <td>
                            <div className="audit-input">1</div>
                          </td>
                        </tr>
                        <tr>
                          <td className="center">2</td>
                          ...
                          <td>
                            <div className="audit-select">同意⌄</div>
                          </td>
                          <td>
                            <div className="audit-input">2</div>
                          </td>
                        </tr>
                      </tbody>
                    </table>
                  ...
                  >
                    ...
                    <button className="audit-danger-btn" type="button">
                      删除议题
                    </button>
                  </div>
                  <div className="audit-inline-row">
                    <span style={{ fontSize: "18px" }}>补充汇报材料</span>
                    <button className="audit-purple-btn" type="button">
                      补充汇报材料
                    </button>
                    <button className="audit-outline-btn" type="button">
                      预览
                    </button>
                  </div>
                  <table className="audit-table">
                  ...
                  <div style={{ margin: "18px 0 12px", fontSize: "18px" }}>
                    综合得分
                  </div>
                  <div className="audit-score-scroll"></div>
                  <div className="audit-field">
                    <label>董监事意见</label>
                    <div className="audit-input muted">未反馈</div>
                  </div>
                  <div className="audit-field" style={{ marginTop: "28px" }}>
                    <label>提请决策事项</label>
                    <div className="audit-input">同意测试议题1</div>
                  </div>
                  <div
                  ...
                  <div className="audit-form-grid audit-vote-form">
                    <div className="audit-field full">
                      <label>投票人：</label>
                      <div className="audit-input"></div>
                    </div>
                    <div className="audit-field full">
                      <label>投票日：</label>
                      <div className="audit-input muted">请选择日期　□</div>
                    </div>
                    <div className="audit-field full">
                      <label>投票方式：</label>
              ...
              <section className="audit-pane" data-audit-pane="department">
                <main className="audit-main">
                  ...
                  <div className="audit-form-grid single">
                    <div className="audit-field">
                      <label>
                        <span className="required">*</span> 期望决策时间
                      </label>
                      <div className="audit-input muted">
                        2026-04-27　　　　　　　　　　　　　　　　　　　　　　　　　　　　　□
                      </div>
                    </div>
                    <div className="audit-field">
                      <label>
                        <span className="required">*</span> 提请决策议题名称
                      </label>
                      <div className="audit-input muted">
                        富维公司测试发送钉钉的议案及表决建议
                      </div>
                ...
                <aside className="audit-side">
                  <div className="audit-status-head">
                    <div className="audit-status-title">当前审批状态</div>
                  </div>
                  <div className="audit-timeline">
                    <div className="audit-time-item orange">
                      <strong>申请人　郑华峰</strong>
                      <br />
                      <br />
                      2026-04-27 10:30:21
                    </div>
                    <div className="audit-time-item green">
                      <strong>科室经理　郑华峰</strong>
                      <span className="audit-pass">审批通过</span>
                      <br />
                      <br />
                      2026-04-28 19:18:09
                      <br />
                    ...
                    <div className="audit-time-item">
                      <strong>抄送　郑华峰</strong>
                      <span className="audit-copy-badge">抄送</span>
                      <br />
                      <br />
                      2026-04-27 10:30:22
                      <br />
              ...
              <section className="audit-pane" data-audit-pane="approval">
                <main className="audit-main">
                  ...
                  <div className="audit-form-grid">
                    <div className="audit-field full">
                      <label>
                        <span className="required">*</span> 议题名称
                      </label>
                      <div className="audit-input">
                        长春富维集团汽车零部件股份有限公司测试发送钉钉议案及表决建议
                      </div>
                    </div>
                    <div className="audit-field">
                      <label>提报人</label>
                      <div className="audit-input muted">郑华峰</div>
                    </div>
                    <div className="audit-field">
                      <label>提报部门</label>
                      <div className="audit-input muted">股权公司</div>
                    </div>
                    <div className="audit-field">
                      <label>提报日期</label>
                      <div className="audit-input muted">2026-05-18</div>
                    </div>
                    <div className="audit-field">
                      <label>分管领导</label>
                      <div className="audit-input">
                        <span className="audit-token">郑华峰</span>
                      </div>
                    </div>
                    <div className="audit-field">
                      <label>列席人</label>
                  ...
                  <div className="audit-approval-extra">
                    <div className="audit-form-grid">
                      <div className="audit-field">
                        ...
                        <div className="audit-radio-row">
                          <span>
                            <span className="audit-radio-dot active"></span>是
                          </span>
                          <span>
                            <span className="audit-radio-dot"></span>否
                          </span>
                        </div>
                      </div>
                      <div className="audit-field">
                        ...
                        >
                          公司所出资的参股公司股东权利所涉及的重大事项（除新增参股公司外）
                        </div>
                      </div>
                      <div className="audit-field">
                        <label>
                    ...
                    <div className="audit-red-copy">
                      相关材料要求：
                      <br />
                      1、所有材料均需解密后上传会议系统
                      <br />
                      2、会议材料中如涉及插入附件，需将附件单独上传
                    </div>

                    <div className="audit-approval-attach">
                      <div className="audit-approval-card full">
                        <div className="audit-approval-card-head">
                          <div className="audit-approval-card-title">
                            业务总监审批
                          </div>
                          <button className="audit-upload-btn" type="button">
                            ↥ 上传文件
                          </button>
                        </div>
                ...
                <aside className="audit-side">
                  <div className="audit-status-head">
                    <div className="audit-status-title">当前审批状态</div>
                    <span className="audit-status-pill">未启动</span>
                  </div>
                </aside>
                <footer className="audit-footer">
                  <button className="foot-btn primary wide" type="button">
              ...
              <section className="audit-pane" data-audit-pane="joint">
                <main className="audit-main">
                  ...
                  <div
                    className="audit-section-title blue"
                    style={{ marginTop: "34px" }}
                  >
                    董事会提请决策事项
                  </div>
                  ...
                  >
                    ...
                    <div className="audit-input muted">测试议题1</div>
                    <button className="audit-danger-btn" disabled type="button">
                      删除议题
                    </button>
                  </div>
                  <div className="audit-inline-row">
                  ...
                  <div style={{ margin: "20px 0 12px", fontSize: "18px" }}>
                    董监事意见
                  </div>
                  <div className="audit-input muted">同意</div>
                  <div style={{ margin: "28px 0 12px", fontSize: "18px" }}>
                    提请决策事项
                  </div>
                  <div className="audit-input muted">同意测试议题2</div>
                </main>
                <aside className="audit-side">
                  <div className="audit-status-head">
                    <div className="audit-status-title">当前审批状态</div>
                  ...
                  <div className="audit-timeline">
                    <div className="audit-time-item">
                      <strong>申请人　孔令娜</strong>
                    </div>
                    <div className="audit-time-item green">
                      <strong>2.三重一大审核　耿姬</strong>
                      <span className="audit-pass">审批通过</span>
                      <br />
                      2026-03-26 16:28:05
                    </div>
                    <div className="audit-time-item green">
                      <strong>13.归档　孔令娜</strong>
                      <span className="audit-pass">审批通过</span>
                    </div>
                  </div>
                </aside>
              ...
              <section className="audit-pane no-side" data-audit-pane="after">
                <main className="audit-main">
                  ...
                  <div className="audit-after-block">
                    <div className="audit-section-title blue">
                      董事会提请决策事项
                    </div>
                    <table className="audit-table">
                      <colgroup>
                  ...
                  <div className="audit-form-grid">
                    <div className="audit-field">
                      <label>议题名称</label>
                      <div className="audit-input muted">
                        国汽（北京）智能网联汽车研究院有限公司第三届董事会第五次会议，2025年度股东会议案
                      </div>
                    </div>
                    <div className="audit-field">
                      <label>提报人</label>
                      <div className="audit-input muted">邵宁</div>
                    </div>
                    <div className="audit-field">
                      <label>联审人员</label>
                      <div className="audit-input muted">陈昊,高峰,刘红艳</div>
                    </div>
                    <div className="audit-field">
                      <label>提报部门</label>
                      <div className="audit-input muted">综合管理部</div>
                    </div>
                    <div className="audit-field">
                      <label>列席人</label>
                      <div className="audit-input muted">
                        <span className="audit-token">滑笑盈</span>
                        <span className="audit-token">黄国平</span>
                        <span className="audit-token">陈昊</span>
                        <span className="audit-token">高峰</span>
                        <span className="audit-token">刘红艳</span>
                      </div>
                    </div>
                    <div className="audit-field">
                      <label>拟上会时间段</label>
                      <div className="audit-input muted">
                        2026-04-09 10:03:11　　　→　2026-04-09 10:06:00　　□
                      </div>
                    </div>
                    <div className="audit-field">
                      <label>相关材料</label>
                      ...
                      <div className="audit-file-row">
                        <span>
                          20260410(股权运营部)国汽智能网联院第三届董事会第五次会议，2025年度股东会议案及表决建议.pdf
                        </span>
                        <span className="audit-file-actions">⊙ ↓</span>
                      </div>
                    </div>
                    <div className="audit-field">
                      <label>预计汇报时长（分钟）</label>
                      <div className="audit-input muted">10</div>
                    </div>
                  </div>
                </main>
                <footer className="audit-footer">
          ...
          <section className="workflow-page" id="workflow-page">
            ...
            <div className="workflow-body">
              <section
                className="workflow-pane workflow-advice-pane active"
                data-workflow-pane="advice"
              >
                <div className="workflow-advice-layout">
                  <div className="workflow-advice-left">
                    ...
                    <div className="workflow-form-stack">
                      <div>
                        <label>会议信息</label>
                        <div className="workflow-textarea">
                          长春富维集团汽车零部件股份有限公司长春一汽富晟集团有限公司、22343、长春一汽富晟集团有限公司于2026年04月21日、2026年04月24日、2026年04月
                        </div>
                      </div>
                      <div>
                        <label>审议情况</label>
                        <div
                          className="audit-red-copy"
                          style={{ margin: "0 0 8px" }}
                        >
                          （请将***年**月**日改为实际日期）
                        </div>
                        <div className="workflow-textarea">
                          一汽股权投资（天津）有限公司分管副总已于xxxx年xx月xx日审议通过相关议题。
                        </div>
                      </div>
                      <div>
                        <label>向董事会发起的建议</label>
                        <div className="workflow-textarea placeholder">
                          请输入
                        </div>
                      </div>
                      <div>
                    ...
                    <table className="audit-table">
                      ...
                      <tbody>
                        <tr>
                          <td className="center">1</td>
                          <td>测试议题1</td>
                          <td style={{ color: "#1f2736", fontWeight: "800" }}>
                            回避表决
                          </td>
                ...
                <div className="workflow-advice-footer">
                  <button className="workflow-download" type="button">
                    下载表决建议单
                  </button>
              ...
              <section
                className="workflow-pane workflow-vote-pane"
                data-workflow-pane="vote"
              >
                ...
                <div className="workflow-vote-upload-grid">
                  <div className="workflow-vote-card">
                    ...
                    <div className="workflow-upload-row">
                      ...
                      <span className="workflow-upload-tip">
                        支持拓展名：.pdf
                      </span>
                    </div>
                    <div className="workflow-upload-row">
                  ...
                  <table className="audit-table workflow-table-wide">
                    ...
                    <tbody>
                      <tr>
                        ...
                        <td>测试议题1</td>
                        <td>
                          <span className="workflow-radio-line">回避表决</span>
                          <span className="workflow-radio-line">同意</span>
                          <span className="workflow-radio-line">
                            有条件同意
                          </span>
                          <span className="workflow-radio-line">不同意</span>
                        </td>
                        <td className="center">-</td>
                        <td>
                          <span className="workflow-radio-line">回避表决</span>
              ...
              <section
                className="workflow-pane workflow-execute-pane"
                data-workflow-pane="execute"
              >
                <div className="workflow-subnav">
                  <button className="active" type="button">
                    <span className="workflow-tab-icon decision"></span>决策情况
                  </button>
                  <button type="button">
                    <span className="workflow-tab-icon assign"></span>议题交办
                  </button>
                </div>
                <div className="workflow-table-wrap">
                  <table className="audit-table workflow-table-wide">
                    ...
                    <thead>
                      <tr>
                        <th rowSpan="2">序号</th>
                        <th rowSpan="2">议题名称</th>
                        <th colSpan="3">总办会</th>
                        <th colSpan="3">三会决议</th>
                        <th rowSpan="2">一致性</th>
                        <th rowSpan="2">原因说明</th>
                      </tr>
                      <tr>
                        <th>董事会</th>
                        <th>监事会</th>
                    ...
                    <tbody>
                      <tr>
                        <td className="center">1</td>
                        <td>测试议题1</td>
                        <td className="center">同意</td>
                        <td className="center">-</td>
                        <td className="center">同意</td>
                        <td className="center">回避表决</td>
                        <td className="center">-</td>
                        <td className="center">同意</td>
                        <td
                          className="center"
                          style={{ color: "#ff2f3d", fontWeight: "800" }}
                        >
                          不一致
                        </td>
                        <td>-</td>
                  ...
                  <div className="finance-meeting-list">
                    <table className="finance-table finance-meeting-table">
                      ...
                      <tbody>
                        <tr>
                          <td>
                            <span className="finance-meeting-type">董事会</span>
                          </td>
                          <td className="center">
                            <span className="finance-meeting-toggle">
                              <span className="finance-switch-track"></span>召开
                            </span>
                          </td>
                          <td>
                            富奥智能转向系统（长春）有限公司第一届董事会第六次会议
                          </td>
                          <td className="center">2026-05-19</td>
                          <td className="center">现场会议</td>
                          <td className="center">2026-06-02 08:30</td>
                          <td className="center">请输入</td>
                        </tr>
                        <tr>
                          <td>
                            <span className="finance-meeting-type">股东会</span>
                          </td>
                          <td className="center">
                            <span className="finance-meeting-toggle">
                              <span className="finance-switch-track"></span>召开
                          ...
                          <td>
                            富奥智能转向系统（长春）有限公司2026年第一次股东会
                          </td>
                          <td className="center">2026-06-02</td>
                          <td className="center">通讯表决</td>
                          <td className="center">2026-06-04</td>
                          <td className="center">请输入</td>
                        </tr>
                      </tbody>
                    </table>
                ...
                <div className="finance-card-body">
                  <table className="finance-table">
                    ...
                    <thead>
                      <tr>
                        <th>
                          <span className="finance-check-cell"></span>
                        </th>
                        <th>序号</th>
                        <th>文件名</th>
                        <th>批注</th>
                    ...
                    <tbody>
                      <tr>
                        ...
                        <td>
                          <button
                            className="finance-file-link"
                            data-open-pdf-editor
                            type="button"
                          >
                            1.png
                          </button>
                        ...
                        <td className="center">
                          <span className="finance-annotation-badge none">
                            不可批注
                          </span>
                        </td>
                      </tr>
                      ...
                      <tr>
                        ...
                        <td>
                          <button
                            className="finance-file-link"
                            data-open-pdf-editor="20250428中联电子议题关键信息页(1).pdf"
                            type="button"
                          >
                        ...
                        <td className="center">
                          <span className="finance-annotation-badge zero">
                            0条批注
                          </span>
                        </td>
                      </tr>
                      <tr>
                        ...
                        <td>
                          <button
                            className="finance-file-link"
                            data-open-pdf-editor="20260403(股权运营部)T3出行第二届董事会2026年第一次会议、2026年第三次临时股东会议案及表决建议.pdf"
                            type="button"
                          >
                            20260403(股权运营部)T3出行第二届董事会2026年第一次会议、2026年第三次临时股东会议案及表决建议.pdf
                          </button>
                        ...
                        <td className="center">
                          <span className="finance-annotation-badge has">
                            3条批注
                          </span>
                        </td>
                      </tr>
        ...
        <section
          className="audit-minutes-overlay"
          id="audit-minutes-overlay"
          aria-hidden="true"
        >
          ...
          <div className="audit-minutes-drawer">
            <header className="audit-minutes-head">
              <button
                className="audit-minutes-close"
                id="audit-minutes-close"
                type="button"
                aria-label="关闭会议纪要"
              ></button>
            ...
            <div className="audit-minutes-body">
              <div className="audit-minutes-tabs">
                <button className="audit-minutes-tab active" type="button">
                  总办会
                </button>
                <button className="audit-minutes-tab" type="button">
                  向分管副总汇报专题会
                </button>
                <button className="audit-minutes-tab" type="button">
                  向总监汇报专题会
              ...
              <div className="audit-minutes-form">
                <div className="audit-minutes-field required">
                  <label>总办会召开日</label>
                  <div className="audit-minutes-date">
                    <span>请选择日期</span>
                    <span>□</span>
                  </div>
                </div>
                <div className="audit-minutes-field">
                  <label>会议纪要</label>
                  <button className="audit-minutes-upload" type="button">
                    ↥ 上传文件
                  </button>
                  <div
                    style={{
        ...
        >
          ...
          <div className="eval-detail-shell">
            <div className="eval-detail-head">
              <button
                className="eval-detail-close"
                id="eval-detail-close"
                type="button"
                aria-label="关闭议题评估详情"
              ></button>
            ...
            <div className="eval-detail-tabs">
              <button
                className="eval-detail-tab active"
                data-eval-detail-tab="execute"
                type="button"
              >
                <span className="eval-tab-icon grade">
                  <span></span>
                </span>
                评估执行
              </button>
              <button
                className="eval-detail-tab"
                data-eval-detail-tab="opinion"
                type="button"
              >
                <span className="eval-tab-icon opinion">
                  <span></span>
                </span>
                综合意见
              </button>
            ...
            <section
              className="eval-detail-pane eval-exec-pane active"
              data-eval-detail-pane="execute"
            >
                      ...
                      <div className="eval-attach-main">
                        ...
                        <div className="eval-attach-meta" id="eval-attach-meta">
                          <div className="eval-attach-row head">
                            <div className="eval-attach-check">
                              <span className="topic-check"></span>
                            </div>
                          ...
                          <div className="eval-attach-row">
                            ...
                            <span className="eval-attach-note-pill yellow">
                              不可批注
                            </span>
                          ...
                          <div className="eval-attach-row">
                            ...
                            <div className="eval-attach-name">
                              1.招标文件-备注版-明阳智能AI在多业务域应用试点项目-20250606.docx
                            </div>
                            <span className="eval-attach-note-pill yellow">
                              不可批注
                          ...
                          <div className="eval-attach-row">
                            ...
                            <div className="eval-attach-name">
                              <button
                                ...
                                data-open-main-pdf="20250428中联电子议题关键信息页(1).pdf"
                                type="button"
                              >
                                20250428中联电子议题关键信息页(1).pdf
                              </button>
                            </div>
                            <span className="eval-attach-note-pill red">
                              0条批注
                            </span>
                            <div className="eval-attach-op">删除</div>
                          </div>
                          <div className="eval-attach-row">
                            ...
                            <div className="eval-attach-name">
                              <button
                                className="pdf-file-link"
                                data-open-main-pdf="20260403(股权运营部)T3出行第二届董事会2026年第一次会议、2026年第三次临时股东会议案及表决建议.pdf"
                                type="button"
                              >
                                20260403(股权运营部)T3出行第二届董事会2026年第一次会议、2026年第三次临时股东会议案及表决建议.pdf
                              </button>
                            </div>
                            <span className="eval-attach-note-pill blue">
                              3条批注
                            </span>
                            <div className="eval-attach-op">删除</div>
                          </div>
                    ...
                    <div className="eval-score-card">
                      <div className="eval-model-block">
                        ...
                        <div className="eval-model-grid">
                          <div className="eval-model-field">
                            <div className="eval-model-field-label">
                              模型目录
                            </div>
                      ...
                      <table className="data-table eval-grade-table">
                        ...
                        <tbody>
                          <tr>
                            ...
                            <td></td>
                            <td>
                              <div className="eval-textbox">通过</div>
                            </td>
                            <td className="rule-cell">
                              符合要求或不涉及，通过；...
                            </td>
                            <td className="result-cell">
                              <span className="eval-radio-outline"></span>不通过
                              <br />
                              <span className="eval-radio-outline active"></span>
                              通过
                            </td>
                            <td className="center">
                              <span className="green-dot"></span>
                          ...
                          <tr>
                            ...
                            <td>
                              <div
                                className="eval-textbox"
                                style={{ borderColor: "#6ea1ff" }}
                              >
                                通过
                              </div>
                            </td>
                      ...
                      <div className="eval-score-summary-row">
                        <div className="eval-grade-note">
                          总得分(1、得分≥80分，议题通过；2、80分&gt;得≥60，议题通过，但要提出管理意见或提示项；3、得分&lt;60
                          分，不通过；4、合规性维度任意一项不通过，议题不通过)
                        </div>
            ...
            <section
              className="eval-detail-pane eval-opinion-pane"
              data-eval-detail-pane="opinion"
            >
              <div className="eval-detail-content">
                <div className="eval-opinion-layout">
                  ...
                  <div className="eval-opinion-form-card">
                    <div className="eval-opinion-section-title eval-opinion-summary-title">
                      综合意见
                    </div>
                    <div className="eval-opinion-form">
                      ...
                      <div className="eval-opinion-radios">
                        ...
                        <span>
                          <span className="eval-radio-outline"></span>
                          有条件同意（附管理建议）
                        </span>
                        <span>
                          <span className="eval-radio-outline"></span>不同意
                        </span>
                      </div>
                      <div style={{ marginBottom: "14px" }}>管理建议</div>
                      <div className="eval-opinion-textarea">
                        请输入管理建议
                      </div>
                    </div>
                  </div>
              ...
              <div className="eval-detail-footer">
                <button
                  className="foot-btn"
                  data-eval-prev="execute"
                  type="button"
                >
                  上一步
                </button>
                <button
                  className="foot-btn primary wide"
                  id="open-eval-supplement"
                  type="button"
                >
                  补充汇报材料
                </button>
            ...
            <section
              className="eval-supplement-overlay"
              id="eval-supplement-overlay"
              aria-hidden="true"
            >
              <div className="eval-fullsheet-head">
                <div className="eval-fullsheet-title-wrap">
                  <button
                    className="eval-fullsheet-close"
                    id="close-eval-supplement"
                    type="button"
                    aria-label="关闭补充汇报材料"
                  ></button>
              ...
              <div className="eval-supplement-body">
                <div className="eval-supplement-steps">
                  <div>
                    ⚠
                    第一步：可以混合上传多张图片和PDF文件。PDF文件会按打印分页自动拆解成多张图片。
                  </div>
                  <div>
                    ⚠ 第二步：通过下面列表的第一列选择本议题展示的材料图片。
                  </div>
                </div>
                <div className="eval-supplement-toolbar">
                  <button className="eval-upload-btn" type="button">
                    上传文件
                  </button>
                </div>
                <div className="eval-supplement-list">
                  <div className="eval-supplement-list-head">
                    <div className="center">
                      <span className="eval-supplement-radio"></span>
                    </div>
                    <div>图片文件名</div>
                    <div>缩略图</div>
                    <div>来源</div>
                  ...
                  <div className="eval-supplement-list-body">
                    <div className="eval-supplement-list-row">
                      ...
                      <div className="left">
                        20250428中联电子议题关键信息页(1)P1.jpg
                      </div>
                      <div className="center">
                        <div className="eval-thumb-preview annotated">
                          <span>批注</span>
                        </div>
                      </div>
                      <div className="center">
                        <span className="eval-source-pill annotated">
                          批注内容
                        </span>
                      </div>
                      <div className="center">
                        <span className="eval-supplement-ops">
                          <span className="move">上移</span>
                          <span className="move">下移</span>
                    ...
                    <div className="eval-supplement-list-row">
                      ...
                      <div className="left">
                        20250428中联电子议题关键信息页(1)P3.jpg
                      </div>
                      <div className="center">
                        <div className="eval-thumb-preview"></div>
                      </div>
                      <div className="center">
                        <span className="eval-source-pill uploaded">
                          手动上传
                        </span>
                      </div>
                      <div className="center">
            ...
            <section
              className="eval-preview-overlay"
              id="eval-preview-overlay"
              aria-hidden="true"
            >
              <div className="eval-fullsheet-head">
                <div className="eval-fullsheet-title-wrap">
              ...
              <div className="eval-preview-body">
                <div className="eval-preview-sheet">
                  <section className="eval-preview-page">
                    <div className="eval-preview-doc-title">
                    ...
                    <div className="eval-preview-section">
                      ...
                      <table className="eval-preview-table">
                        ...
                        <tbody>
                          <tr>
                            <td>1</td>
                            <td className="left">gitlab.jpg</td>
                          </tr>
                        </tbody>
                      </table>
                    </div>
                    ...
                    <div className="eval-preview-section">
                      <div className="eval-preview-section-title">
                        风控合规审核意见
                      </div>
                    </div>
                      ...
                      <table className="eval-preview-table">
                        ...
                        <tbody>
                          ...
                          <tr>
                            ...
                            <td>内部管理规定</td>
                            <td>/</td>
                            <td className="left">该公司内部该类事项管理要求</td>
                            <td>1</td>
                            <td className="left">
                              符合要求或不涉及，通过；不符合要求，否决该议案
                            </td>
                          ...
                          <tr>
                            ...
                            <td>控股股东要求</td>
                            <td>/</td>
                            <td className="left">控股股东该类事项要求</td>
                            <td>1</td>
                            <td className="left">
                              符合要求或不涉及，通过；不符合要求，否决该议案
                            </td>
                          ...
                          <tr>
                            ...
                            <td>审议程序</td>
                            <td>/</td>
                            <td className="left">是否按制度要求进行前置审议</td>
                            <td>1</td>
                            <td className="left">
                              符合要求或不涉及，通过；不符合要求，否决该议案
                            </td>
                          ...
                          <tr>
                            ...
                            <td>111</td>
                            <td className="left">
                              开展效果达到目标，得100分；开展效果未达到目标，得0分
                            </td>
                            <td>1</td>
                            <td>
                              <span
                                className="green-dot"
                                style={{ background: "#f31e1e" }}
                              ></span>
                            </td>
                          </tr>
                          <tr>
                            <td colSpan="3">综合得分</td>
                            <td colSpan="5" className="left">
                              1、得分≥80分，议题通过；2、80分&gt;得≥60，议题通过，但要提出管理意见或提示项；3、得分&lt;60分，不通过；4、合规性维度任意一项不通过，议题不通过
                            </td>
                            <td>1</td>
                            <td></td>
                          </tr>
                              ...
                              <div className="eval-preview-image-box">
                                <div className="eval-preview-proof-doc">
                                  ...
                                  <div className="eval-preview-proof-panel">
                                    ...
                                    <div
                                      className="eval-preview-proof-region"
                                      style={{
                                        right: "10%",
                                        top: "10%",
                                        width: "38%",
                              ...
                              <article className="eval-preview-side-card">
                                <div className="eval-preview-side-tags">
                                  <div className="eval-preview-side-tag-group">
                                    <span className="eval-preview-side-tag page">
                                      第1页
                                    </span>
                                    <span className="eval-preview-side-tag type">
                                      框选批注
                                    </span>
                                  </div>
                                </div>
                                <div className="eval-preview-side-copy">
                                  关键净值与拆除费用表格区域，需要在评估前补充附件来源说明。
                                </div>
                                <div className="eval-preview-side-meta">
                                  <span>郑华峰 2025-06-21 19:05</span>
                                  <span className="eval-preview-side-actions">
                        ...
                        <div className="eval-preview-proof-row">
                          <article className="eval-preview-proof-image">
                            <div className="eval-preview-pdf-sheet">
                              ...
                              <div className="eval-preview-image-box">
                                <div className="eval-preview-proof-doc">
                                  <div className="eval-preview-proof-summary-title">
                                    补充评估说明
                                  </div>
                                  <div className="eval-preview-proof-doc-rule"></div>
                                  <div className="eval-preview-proof-section">
            ...
            <div
              className="eval-picker-modal"
              id="eval-attach-modal"
              aria-hidden="true"
            >
              <div className="eval-picker-mask" data-close-eval-modal></div>
              <div className="eval-picker-shell">
                <div className="eval-picker-head">
                  <div className="eval-picker-title" id="eval-attach-title">
                    附件确认
                  </div>
                  <button
                    className="eval-detail-close"
                    data-close-eval-modal
                    type="button"
                    aria-label="关闭附件确认"
                  ></button>
                </div>
                <div className="eval-picker-body">
                  <div className="eval-modal-toolbar" id="eval-attach-toolbar">
                    <button
                      className="foot-btn primary"
                      type="button"
                      id="eval-attach-toolbar-btn"
                    >
                      关联附件
                    </button>
                  </div>
                  <div className="eval-associate-note" id="eval-attach-note">
                    请勾选当前议题需要关联的附件，PDF 文件可点击进入批注编辑。
                  </div>
                  <table
                    className="data-table eval-file-table"
                    id="eval-attach-table"
                  >
                    <colgroup>
                      <col style={{ width: "54px" }} />
                      <col style={{ width: "84px" }} />
                    ...
                    <tbody>
                      <tr>
                        ...
                        <td className="center">1</td>
                        <td className="blue-link file-name">1.png</td>
                        <td className="danger center">删除</td>
                      </tr>
                      <tr>
                        <td className="center">
                          <span className="topic-check"></span>
                        ...
                        <td className="center">2</td>
                        <td className="blue-link file-name">
                          1.招标文件-备注版-明阳智能AI在多业务域应用试点项目-20250606.docx
                        </td>
                        <td className="danger center">删除</td>
                      </tr>
            ...
            >
              ...
              <div className="eval-picker-shell">
                ...
                <div className="eval-picker-body">
                  <div
                    className="eval-filter-row"
                    style={{ position: "static" }}
                  >
                    <div className="eval-filter-field">
                      <label>议题分类（大）</label>
                      <div className="fake-select">
                        1.经营类<span className="chev"></span>
                      </div>
                    </div>
                    <div className="eval-filter-field">
                      <label>议题分类（中）</label>
                      <div className="fake-select">
                        1.3 定期监管报告<span className="chev"></span>
                      </div>
                    </div>
                    <div className="eval-filter-field">
                      <label>议题分类（小）</label>
                      <div className="fake-select">
                        ebd442f3b57c4308ba5faaaad44b15db
                        <span className="chev"></span>
                      </div>
                    </div>
                    <div className="eval-filter-actions">
                      <button className="eval-outline-btn" type="button">
                        重置
                      </button>
                      <button className="eval-search-btn" type="button">
                        查询
                      </button>
                    </div>
                  </div>
                  <table
                    className="data-table eval-model-table"
                  ...
                  >
                    ...
                    <tbody>
                      <tr className="eval-model-row-selected">
                        ...
                        <td className="center">
                          <span
                            ...
                            style={{
                              ...
                              color: "#44c6d9",
                              background:
                                "linear-gradient(180deg,#e5fbef,#e5fbef)",
                            }}
                          >
                            有效
                          </span>
                        ...
                        <td className="center">
                          <span className="modal-op">
                            <span className="eval-op">查看</span>
                            <span className="eval-op" data-close-eval-modal>
                              更换模型
                            </span>
                          </span>
                        </td>
                  ...
                  <div
                    className="eval-model-selected"
                    style={{ left: "28px", top: "auto", bottom: "84px" }}
                  ></div>
                  <div className="eval-picker-footer">
                    <button
                      className="foot-btn"
            ...
            <section
              ...
              id="pdf-editor-overlay"
              aria-hidden="true"
            >
              <div className="pdf-editor-head">
                <button
                  className="pdf-editor-back"
                  id="pdf-editor-back"
                  type="button"
                  aria-label="返回附件确认"
                ></button>
              ...
              <div className="pdf-toolbar">
                <div className="pdf-toolbar-label" id="pdf-toolbar-label">
                  标注模式：
                </div>
                <label className="pdf-page-link-toggle">
                  关联此页
                  <input type="checkbox" id="pdf-page-link-toggle" />
                </label>
                <div className="pdf-toolbar-switch">
                  <button
                    className="pdf-mode-btn annotation-only"
                    data-pdf-mode="all"
                    type="button"
                  >
                    全部页面
                  ...
                  <button
                    className="pdf-mode-btn attach-only active"
                    data-pdf-mode="area"
                    type="button"
                  >
                    区域标记
                  ...
                  <button
                    className="pdf-mode-btn attach-only"
                    data-pdf-mode="text"
                    type="button"
                  >
                    文字选择
                ...
                <div className="pdf-toolbar-hint" id="pdf-toolbar-hint">
                  拖动鼠标绘制矩形区域
                </div>
                <button
                  className="pdf-toolbar-cancel"
                  id="pdf-toolbar-cancel"
                  type="button"
                >
                  取消
                </button>
              ...
              <div className="pdf-editor-body">
                <section className="pdf-main">
                  <div className="pdf-scroll" id="pdf-scroll">
                    <article
                      className="pdf-page active"
                      data-pdf-page="1"
                      data-pdf-scope="full annotated"
                    >
                      ...
                      <p className="pdf-paragraph">
                        一汽解放汽车有限公司发动机分公司向我公司转让31项报废设备，主要为报废清洗机、磨床、车床、抛光机、连杆螺母拧紧机等资产，
                        <span
                          className="pdf-selectable"
                          data-selectable-id="text-1"
                          data-page="1"
                        >
                          全部资产均已拆除完毕，存放在解放卡车厂院内，根据现场实际情况判断，均已无法再使用。
                        </span>
                    ...
                    >
                      ...
                      <p className="pdf-paragraph">
                        <span
                          className="pdf-selectable"
                          data-selectable-id="discussion-1"
                          data-page="2"
                        >
                          建议在董事会审议前完成净值口径、处置价格依据及资产完备性说明的补充标注，并同步形成任务清单，便于后续跟踪。
                        </span>
                    ...
                    >
                      ...
                      <p className="pdf-paragraph">
                        <span
                          className="pdf-selectable"
                          data-selectable-id="text-4"
                          data-page="4"
                        >
                          本页包含与资产定价依据相关的补充说明，可用于关联批注演示。
                        </span>
                      </p>
                      <div
                        className="pdf-canvas idle"
                    ...
                    >
                      ...
                      <p className="pdf-paragraph">
                        <span
                          className="pdf-selectable"
                          data-selectable-id="text-7"
                          data-page="7"
                        >
                          本页标注资产净值与拆除费用之间的异常说明，便于关联到评分维度。
                        </span>
                      </p>
                      <div
                        className="pdf-canvas idle"
                    ...
                    >
                      ...
                      <p className="pdf-paragraph">
                        <span
                          className="pdf-selectable"
                          data-selectable-id="text-9"
                          data-page="9"
                        >
                          本页标注管理建议相关段落，便于在综合意见环节快速引用。
                        </span>
                      </p>
                      <div
                        className="pdf-canvas idle"
                  ...
                  <div className="pdf-page-jump">
                    ...
                    <div className="pdf-page-stepper">
                      ...
                      <input
                        ...
                        inputMode="numeric"
                      />
                      <span className="pdf-page-total" id="pdf-page-total">
                        / 10
                      </span>
                      <button
                        className="pdf-page-step-btn"
                  ...
                  <div
                    className="pdf-note-pop"
                    id="pdf-note-pop"
                    style={{ display: "none" }}
                  >
                    <div className="pdf-note-title">填写说明</div>
                    <textarea
                      className="pdf-note-input"
                      id="pdf-note-input"
                      placeholder="请输入批注说明"
                    ></textarea>
                    <div className="pdf-note-type">
                      ...
                      <div className="pdf-note-type-options">
                        <label className="pdf-note-type-option">
                          <input
                            type="radio"
                            name="pdf-note-need-reply"
                            defaultValue="yes"
                          />
                          是
                        </label>
                  ...
                  <div className="pdf-bottom-dock" id="pdf-bottom-dock">
                    <div className="pdf-bottom-info-row">
                      <div className="pdf-bottom-info">
                        ...
                        <div className="pdf-bottom-info-value">
                          外部管理规定
                        </div>
                      </div>
                      <div className="pdf-bottom-info">
                        <div className="pdf-bottom-info-label">
                          参股公司信息
                        </div>
                        <div className="pdf-bottom-info-value">
                          长春富维集团汽车零部件股份有限公司
                    ...
                    >
                      <div className="pdf-bottom-attach-list">
                        <div className="pdf-bottom-attach-row">
                          ...
                          <button
                            className="pdf-bottom-row-compare"
                            type="button"
                          >
                            对比
                          </button>
                    ...
                    <div className="pdf-bottom-actions-row">
                      <button className="pdf-bottom-action" type="button">
                        参股公司信息
                      </button>
                      <button className="pdf-bottom-action" type="button">
                        一企一策
                ...
                <aside className="pdf-side">
                  <div className="pdf-side-head">
                    <div className="pdf-side-title" id="pdf-side-title">
                      批注列表
                    </div>
                    <div className="pdf-side-actions">
                      <button
                        ...
                        aria-label={
                          pdfFavorited ? "取消收藏当前 PDF" : "收藏当前 PDF"
                        }
                  ...
                  <div className="pdf-task-scroll">
                    <div className="pdf-task-list" id="pdf-task-list">
                      <article
                        className="pdf-task-card active"
                        data-annotation-card="area-1"
                      >
                      ...
                      <article
                        className="pdf-task-card"
                        data-annotation-card="text-1"
                      >
                        <div className="pdf-task-top">
                          <div className="pdf-task-tags">
                            <span className="pdf-tag page">第1页</span>
                      ...
                      <article
                        className="pdf-task-card"
                        data-annotation-card="discussion-1"
                      >
                        <div className="pdf-task-top">
                          <div className="pdf-task-tags">
                        ...
                        <div className="pdf-task-discussion">
                          <div className="pdf-discussion-summary">
                            <span>共 2 轮讨论</span>
                            <button
                              className="pdf-discussion-toggle"
                              type="button"
                              data-expand-text="展开更早 1 轮讨论"
                              data-collapse-text="收起更早讨论"
                            >
                              展开更早 1 轮讨论
                            </button>
                          </div>
                            ...
                            <div className="pdf-discussion-round">
                              <div className="pdf-discussion-message suggestion">
                                <div className="pdf-discussion-head">
                                  吴文君 · 建议 · 2026-05-15 14:30
                                </div>
                                <div className="pdf-discussion-body">
                                  建议补充现场照片作为支撑，避免“无法再使用”的判断只停留在文字描述。
                                </div>
                              </div>
                              <div className="pdf-discussion-message reply">
                                <div className="pdf-discussion-head">
                                  创建人 · 回复 · 2026-05-15 15:02
                                </div>
                                <div className="pdf-discussion-body">
                                  已补充现场照片，后续会把对应页码同步到汇报材料里，方便会上直接引用。
                                </div>
                              </div>
                            </div>
                          </div>
                          <div className="pdf-discussion-latest">
                            <div className="pdf-discussion-round">
                              <div className="pdf-discussion-message suggestion">
                                <div className="pdf-discussion-head">
                                  郑华峰 · 建议 · 2026-05-15 17:18
                                </div>
                                <div className="pdf-discussion-body">
                                  建议把净值口径、处置价格依据和资产完备性说明拆成三条附件来源，便于风控复核。
                                </div>
                              </div>
                              <div className="pdf-discussion-message reply">
                                <div className="pdf-discussion-head">
                                  创建人 · 回复 · 2026-05-15 18:06
                                </div>
                                <div className="pdf-discussion-body">
                                  收到，我会按三项拆分成独立补充材料，并在汇报稿里逐条挂接到对应评价要素。
                                </div>
                              </div>
                            </div>
                          </div>
                          ...
                          <div
                            className="pdf-discussion-composer"
                ...
                <section className="pdf-compare-inline" id="pdf-compare-inline">
                    ...
                    <div className="pdf-compare-doc">
                      <div className="pdf-compare-doc-head">
                        <div className="pdf-compare-doc-meta">
                          <div className="pdf-compare-doc-title">
                            20250428中联电子议题关键信息页(1).pdf
                          </div>
                          <select
                            className="pdf-compare-switch"
                            data-compare-switch="1"
                          >
                            <option>
                              20250428中联电子议题关键信息页(1).pdf
                            </option>
                      ...
                      <div className="pdf-compare-page">
                        ...
                        <div className="pdf-page-rule"></div>
                        <p className="pdf-paragraph">
                          全部资产均已拆除完毕，存放在解放卡车厂院内，根据现场实际情况判断，均已无法再使用。评估前需补充附件来源说明。
                        </p>
                        <table className="pdf-table">
                          <thead>
                            <tr>
                  ...
                  <div className="pdf-compare-panel">
                    <div className="pdf-compare-doc">
                      <div className="pdf-compare-doc-head">
                        <button
                          className="pdf-compare-back"
                          id="pdf-compare-inline-back"
                          type="button"
                          aria-label="返回当前编辑页"
                        >
                          返回
            ...
            <div
              className="pdf-compare-overlay"
              id="pdf-compare-overlay"
              aria-hidden="true"
            >
              <div className="pdf-compare-mask"></div>
              <div className="pdf-compare-drawer">
                <button
                  className="pdf-compare-close"
                  id="pdf-compare-close"
                  type="button"
                  aria-label="关闭对比抽屉"
                ></button>
                <div className="pdf-compare-panel">
                  <div className="pdf-compare-doc">
                    ...
                    <div className="pdf-compare-pager">
                      <button className="pdf-page-nav-btn" type="button">
                        上一页
                      </button>
                      <div className="pdf-page-stepper">
                        <button className="pdf-page-step-btn" type="button">
                          -
                        </button>
                        <input
                          className="pdf-page-input"
